function OpinnotPage() {
  function resolveOpiskeluoikeus(indexOrName, omatTiedot) {
    ...
    if (typeof indexOrName === 'undefined') {
      ...
      if (all.length !== 1) {
        throw new Error(
          'opiskeluoikeus not found, expected one, got ' + all.length
        )
      }
      return all
    } else if (typeof indexOrName === 'string') {
      // find the right opiskeluoikeus by name (safe against changes in sorting)
      var matching = all.filter(function (i, v) {
        return $(v)
          .find(
            !omatTiedot
              ? '.otsikkotiedot'
              : 'button.opiskeluoikeus-button .opiskeluoikeus-button-content .opiskeluoikeus-title'
          )
          .text()
          .includes(indexOrName)
      })
      if (matching.length !== 1) {
        throw new Error(
          'opiskeluoikeus not found, got ' +
            matching.length +
            ' matches for ' +
            indexOrName
        )
      ...
      throw new Error(
        'opiskeluoikeus not found, unknown type ' + typeof indexOrName
      )
    }
  }
  var api = {
    valitseOmatTiedotOpiskeluoikeus: function (indexOrName) {
      return function () {
        ...
        return click(
          findSingle('button.opiskeluoikeus-button', opiskeluoikeus)
        )()
      }
    },
    valitseOmatTiedotOpiskeluoikeudenLukionSuoritus: function (
      indexOrName,
      suoritus
    ) {
      return function () {
        ...
        return click(
          findSingle(
            'table.omattiedot-suoritukset tr.oppiaine-header td.oppiaine:contains(' +
              suoritus +
              ')',
            opiskeluoikeus
          )
    ...
    getOpiskeluoikeudenTila: function (indexOrName) {
      ...
      return findSingle(
        '.opiskeluoikeuden-tiedot .property.tila .value',
        opiskeluoikeus
      )().text()
    },
    ...
    getSuoritustapa: function (indexOrName) {
      var opiskeluoikeus = resolveOpiskeluoikeus(indexOrName)
      return opiskeluoikeus
        .find('.suoritus .property.suoritustapa .value')
        .text()
    },
    getKoulutusModuuli: function (indexOrName) {
      ...
      return {
        nimi: opiskeluoikeus
          .find(
            '.suoritus .property.koulutusmoduuli .koulutusmoduuli .tunniste .nimi .value'
          )
          .text(),
        koodi: opiskeluoikeus
          .find(
            '.suoritus .property.koulutusmoduuli .koulutusmoduuli .tunniste .koodiarvo .value'
          )
          .text(),
        kuvaus: opiskeluoikeus
          .find(
            '.suoritus .property.koulutusmoduuli .koulutusmoduuli .properties .kuvaus .value'
          )
          .text()
      }
    },
    ...
    getSuorituskieli: function (indexOrName) {
      var opiskeluoikeus = resolveOpiskeluoikeus(indexOrName)
      return opiskeluoikeus
        .find('.suoritus .property.suorituskieli .value')
        .text()
    },
    valitseSuoritus: function (indexOrName, nimi, omatTiedot) {
      ...
      return function () {
        ...
        var tab = findFirst(
          '.suoritus-tabs > ul > li:contains(' + nimi + ')',
          opiskeluoikeus
        )()
    ...
    suljeLisaysDialogi: function () {
      if (S('.lisaa-opiskeluoikeusjakso-modal .modal-content').is(':visible')) {
        return click(
          S('.opiskeluoikeuden-tiedot .modal-content .close-modal')
        )()
      }
    },
    ...
    lisääDIAValmistavaOppiaineOsasuoritus: function () {
      return seq(
        wait.untilVisible(function () {
          return S('.uusi-kurssi > a')
        }),
        click('.uusi-kurssi > a'),
        wait.untilVisible(function () {
          return S('#modal-main-content > .kurssi')
        }),
        ...
        wait.until(function () {
          return !isElementVisible(S('#modal-main-content > .kurssi'))
        })
      )()
    },
    haeValmistavanOppiaineenOsasuoritustenVaihtoehtojenLukumäärä: function () {
      return new Promise(function (resolve) {
        seq(
          ...
          click('.uusi-kurssi .dropdown input'),
          wait.untilVisible(function () {
            return S('.uusi-kurssi .modal-content')
          }),
          function () {
            return resolve(
              S('.uusi-kurssi .dropdown > .options .option').length
            )
          }
        )()
      })
    ...
    invalidateOpiskeluoikeusIsShown: function () {
      return isElementVisible(
        findSingle('.invalidate.invalidate-opiskeluoikeus')
      )
    },
    confirmInvalidateOpiskeluoikeusIsShown: function () {
      return isElementVisible(
        findSingle('.confirm-invalidate.invalidate-opiskeluoikeus__confirm')
      )
    },
    deletePäätasonSuoritusIsShown: function () {
      return isElementVisible(
        findSingle('.invalidate.delete-paatason-suoritus')
      )
    },
    confirmDeletePäätasonSuoritusIsShown: function () {
      return isElementVisible(
        findSingle('.confirm-invalidate.delete-paatason-suoritus__confirm')
      )
    },
    invalidateOpiskeluoikeus: click(
      findSingle('.invalidate.invalidate-opiskeluoikeus')
    ),
    confirmInvalidateOpiskeluoikeus: click(
      findSingle('.confirm-invalidate.invalidate-opiskeluoikeus__confirm')
    ),
    hideInvalidateMessage: click(findSingle('.hide-invalidation-notification')),
    deletePäätasonSuoritus: click(
      findSingle('.invalidate.delete-paatason-suoritus')
    ),
    confirmDeletePäätasonSuoritus: click(
      findSingle('.confirm-invalidate.delete-paatason-suoritus__confirm')
    ),
  ...
  var api = {
    ...
    uusiOppiaine: function (selector) {
      ...
      return OpinnotPage()
        ...
        .propertyBySelector(selector + ' .uusi-oppiaine')
    },

    oppiaine: function (indexOrClass) {
      var oppiaineElem =
        typeof indexOrClass === 'number'
...
function Oppiaine(oppiaineElem) {
  ...
  var oppiaineApi = _.merge(
    {
      ...
      avaaAlkuvaiheenLisääKurssiDialog: click(
        findSingle('.uusi-alkuvaiheen-kurssi a', oppiaineElem)
      ),
      lisääKurssi: function (kurssi, tyyppi) {
        return seq(
          oppiaineApi.avaaLisääKurssiDialog,
          oppiaineApi.lisääKurssiDialog.valitseKurssi(
            kurssi || 'Kieli ja kulttuuri'
          ),
          oppiaineApi.lisääKurssiDialog.valitseKurssinTyyppi(
            tyyppi || 'Pakollinen'
          ),
          oppiaineApi.lisääKurssiDialog.lisääKurssi
        )
      },
      lisääLaajuudellinenOpintojakso: function (opintojakso, laajuus) {
        return seq(
          oppiaineApi.avaaLisääKurssiDialog,
          oppiaineApi.lisääKurssiDialog.valitseKurssi(
            opintojakso || 'Kieli ja kulttuuri'
          ),
          oppiaineApi.lisääKurssiDialog.asetaLaajuus(laajuus || '2'),
          oppiaineApi.lisääKurssiDialog.lisääKurssi
        )
      },
      lisääPaikallinenKurssi: function (tunniste, nimi, kuvaus) {
        return seq(
          oppiaineApi.avaaLisääKurssiDialog,
      ...
      lisääLaajuudellinenPaikallinenKurssi: function (
        tunniste,
        nimi,
        kuvaus,
        laajuus
      ...
      kurssi: function (identifier) {
        return Kurssi(
          subElement(oppiaineElem, '.kurssi:contains(' + identifier + ')')
        )
      },
      ...
      errorText: function () {
        return extractAsText(subElement(oppiaineElem, '> .error'))
      },
      ...
      koetuloksenNelinkertainenPistemäärä: editorApi.propertyBySelector(
        'tr.koetuloksenNelinkertainenPistemäärä'
      ),
      expandable: function () {
        return (
          typeof findFirstNotThrowing('.toggle-expand', oppiaineElem) !==
          'undefined'
        )
      },
  ...
  function LisääKurssiDialog() {
    ...
    function kurssiDropdown() {
      return api.propertyBySelector('.kurssi')
    }
    function kurssinTyyppiDropdown() {
      return api.propertyBySelector('.kurssinTyyppi')
    }

    /* Paikallinen kurssi */
    function tunniste() {
      return api.propertyBySelector('.koodiarvo')
    }
    function nimi() {
      return api.propertyBySelector('.nimi')
    }
    function kuvaus() {
      return api.propertyBySelector('.kuvaus')
    }
...
function Kurssi(elem) {
  ...
  var api = {
    ...
    lasketaanKokonaispistemäärään: Editor(elem).propertyBySelector(
      'tr.lasketaanKokonaispistemäärään'
    ),
    lisääTunnustettu: click(subElement(detailsElem, '.tunnustettu .add-value')),
    poistaTunnustettu: click(
      subElement(detailsElem, '.tunnustettu .remove-value')
    ),
...
function TutkinnonOsat(groupId, base) {
  ...
  var uusiTutkinnonOsaElement = findSingle(
    withSuffix('.uusi-tutkinnon-osa'),
    base
  )
  ...
  return {
    ...
    tutkinnonOsa: function (tutkinnonOsaIndex) {
      var tutkinnonOsaElement = findSingle(
        withSuffix('.tutkinnon-osa') + ':eq(' + tutkinnonOsaIndex + ')',
        base
      )
      ...
      var api = _.merge(
        {
          ...
          toggleExpand: click(
            findSingle('.suoritus .toggle-expand', tutkinnonOsaElement)
          ),
          lisääOsaamisenTunnustaminen: click(
            findSingle('.tunnustettu .add-value', tutkinnonOsaElement)
          ),
          poistaOsaamisenTunnustaminen: click(
            findSingle('.tunnustettu .remove-value', tutkinnonOsaElement)
          ),
          poistaLisätieto: click(
            findSingle('.lisätiedot .remove-item', tutkinnonOsaElement)
          ),
          ...
          arviointiNth: function (nth) {
            return api.propertyBySelector(
              '.arviointi > .value > ul > li:nth-child(' + (nth + 1) + ')'
            )
          },
          ...
          lueNäyttöModal: function () {
            function extractDropdownArray(elem) {
              return elem
                .find('ul.array > li')
                .map(function () {
                  return Page(this).getInput('.dropdown').value()
                })
                .get()
                .slice(0, -1)
            }
            return {
              arvosana: Page(tutkinnonOsaElement)
                .getInput('.näyttö .modal-content .arvosana .value .dropdown')
                .value(),
              arviointipäivä: Page(tutkinnonOsaElement)
                .getInput('.näyttö .modal-content .päivä .value input')
                .value(),
              kuvaus: Page(tutkinnonOsaElement)
                .getInput('.näyttö .modal-content .kuvaus .value textarea')
                .value(),
              arvioinnistaPäättäneet: extractDropdownArray(
                S(
                  '.näyttö .modal-content .arvioinnistaPäättäneet .value',
                  tutkinnonOsaElement
                )
              ),
              arviointikeskusteluunOsallistuneet: extractDropdownArray(
                S(
                  '.näyttö .modal-content .arviointikeskusteluunOsallistuneet .value',
                  tutkinnonOsaElement
                )
              ),
              suorituspaikka: [
                ...
                Page(tutkinnonOsaElement)
                  .getInput(
                    '.näyttö .modal-content .suorituspaikka .value input:not(.select)'
                  )
                  .value()
              ],
              työssäoppimisenYhteydessä: Page(tutkinnonOsaElement)
                .getInput(
                  '.näyttö .modal-content .työssäoppimisenYhteydessä .value input'
                )
                .value()
            }
          },
          avaaNäyttöModal: function () {
            ...
            return Q()
              .then(
                click(
                  findSingle(
                    '.näyttö .' + (valueExists ? 'edit' : 'add') + '-value',
                    tutkinnonOsaElement
                  )
                )
              )
              .then(
                wait.untilVisible(
                  findSingle('.lisää-näyttö-modal .modal-content')
                )
              )
          },
          asetaNäytönTiedot: function (tiedot) {
            return function () {
              return wait
                ...
                .then(function () {
                  // Normalize state
                  var addVButton = tutkinnonOsaElement().find(
                    '.näyttö .modal-content .arviointi .add-value'
                  )
                  if (addVButton.length) {
                    ...
                    tutkinnonOsaElement()
                      .find(
                        '.näyttö .modal-content .arvioinnistaPäättäneet .value li .remove-item:visible'
                      )
                      .each(function (i, e) {
                        click(e)()
                      })
                    tutkinnonOsaElement()
                      .find(
                        '.näyttö .modal-content .arviointikeskusteluunOsallistuneet .value li .remove-item:visible'
                      )
                      .each(function (i, e) {
                        click(e)()
                      })
                ...
                .then(function () {
                  Page(tutkinnonOsaElement)
                    .getInput(
                      '.näyttö .modal-content .arvosana .value .dropdown'
                    )
                    .setValue(tiedot.arvosana, 1)
                  Page(tutkinnonOsaElement)
                    .getInput('.näyttö .modal-content .päivä .value input')
                    .setValue(tiedot.arviointipäivä)
                  Page(tutkinnonOsaElement)
                    .getInput('.näyttö .modal-content .kuvaus .value textarea')
                    .setValue(tiedot.kuvaus)
                  tiedot.arvioinnistaPäättäneet.map(function (v, i) {
                    Page(tutkinnonOsaElement)
                      .getInput(
                        '.näyttö .modal-content .arvioinnistaPäättäneet .value li:' +
                          (i === 0 ? 'first' : 'last') +
                          '-child .dropdown'
                      )
                      .setValue(v)
                  ...
                  Page(tutkinnonOsaElement)
                    ...
                    .setValue(tiedot.suorituspaikka[0])
                  Page(tutkinnonOsaElement)
                    .getInput(
                      '.näyttö .modal-content .suorituspaikka .value input:not(.select)'
                    )
                    .setValue(tiedot.suorituspaikka[1])
                  Page(tutkinnonOsaElement)
                    .getInput(
                      '.näyttö .modal-content .työssäoppimisenYhteydessä .value input'
                    )
                    .setValue(tiedot.työssäoppimisenYhteydessä)

                  if (
                    findSingle(
                      '.näyttö .modal-content button.vahvista',
                      tutkinnonOsaElement
                    )().prop('disabled')
                  ) {
          ...
          poistaNäyttö: click(
            findSingle('.näyttö .remove-value', tutkinnonOsaElement)
          )
        },
        {},
    ...
    lisääKorkeakouluopintoja: function () {
      return click(
        subElement(uusiTutkinnonOsaElement, '.korkeakouluopinto a')
      )()
    },
      ...
      function () {
        return click(
          subElement(
            ...
            '.jatkoopintovalmiuksiatukevienopintojensuoritus a'
          )
        )()
      },
    lisääLukioOpinto: function (nimi) {
      return lisääPaikallinenTutkinnonOsa(
        nimi,
        '.lisaa-lukio-opinto-modal',
        '.lukio-opinto a'
      )
    },
    lisääMuuOpintovalmiuksiaTukevaOpinto: function (nimi) {
      return lisääPaikallinenTutkinnonOsa(
        nimi,
        '.lisaa-muu-opintovalmiuksia-tukeva-opinto-modal',
        '.muu-opintovalmiuksia-tukeva-opinto a'
      )
    },
    ...
    lisääTutkinnonOsaToisestaTutkinnosta: function (tutkinto, nimi) {
      return function () {
        var modalElement = subElement(
          uusiTutkinnonOsaElement,
          '.osa-toisesta-tutkinnosta .modal'
        )
        return click(
          subElement(uusiTutkinnonOsaElement, '.osa-toisesta-tutkinnosta a')
        )()
          .then(
            Page(modalElement).setInputValue(
              '.tutkinto .autocomplete',
          ...
          .then(
            Page(modalElement).setInputValue('.tutkinnon-osat .dropdown', nimi)
          )
          .then(
            click(subElement(modalElement, 'button.vahvista:not(:disabled)'))
          )
  ...
  var api = {
    lisääOsaamiskokonaisuus: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vst-osaamiskokonaisuus',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    lisääSuuntautumisopinto: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vst-suuntautumisopinnot',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    lisääMuuallaSuoritettuOpinto: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vst-muutopinnot',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    lisääPaikallinen: function (nimi) {
      return function () {
        var modalElement = findSingle(
          '.lisaa-paikallinen-vst-suoritus-modal',
          selectedOsasuoritus
        )
    ...
    lisääLukutaitokoulutuksenKokonaisuus: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vst-lukutaitokoulutuksenkokonaisuudensuoritus',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    lisääKieliJaViestintäosaamisenOsasuoritus: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vstkoto2022-kielijaviestinta',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    lisääYhteiskuntaJaTyöelämänOsaamisenOsasuoritus: function (hakusana) {
      return function () {
        return Page(
          findSingle(
            '.lisaa-uusi-suoritus.vstkoto2022-yhteiskuntajatyoosaamis',
            selectedOsasuoritus
          )
        )
          .setInputValue('.dropdown, .autocomplete', hakusana)()
    ...
    enterLaajuus: function (osasuorituksenNimi, arvo) {
      return function () {
        return VSTSuoritukset()
          .selectOsasuoritus(osasuorituksenNimi)()
          .property('laajuus')
          .setValue(arvo)()
      }
    },
    selectArvosana: function (osasuorituksenNimi, arvo) {
      return function () {
        return VSTSuoritukset()
          .selectOsasuoritus(osasuorituksenNimi)()
          .firstPropertyBySelector('tr td.arvosana')
          .selectValue(arvo)()
      }
    },
    selectOsasuoritus: function (nimi) {
      return function () {
        var osasuoritukset = selectedOsasuoritus
          ? selectedOsasuoritus.find('.vst-osasuoritus')
          : S('.vst-osasuoritus')
        ...
        osasuoritukset.each(function (i, e) {
          if (
            extractAsText(S(e).find('tr > td.suoritus > .nimi')).includes(nimi)
          ) {
            found = S(e)
          }
        })
...
function TUVASuoritukset(prev) {
  ...
  var api = {
    ...
    lisääArvosana: function (hakusana, selector) {
      var _selector = selector || '.arvosana .dropdown-wrapper'
      return function () {
    ...
    lisääValinnainenPaikallinenSuoritus: function (nimi) {
      return function () {
        var modalElement = findSingle(
          '.lisaa-paikallinen-tuva-suoritus-modal',
          selectedOsasuoritus
        )
    ...
    selectOsasuoritus: function (nimi) {
      return function () {
        var osasuoritukset = selectedOsasuoritus
          ? selectedOsasuoritus.find('.tuva-osasuoritus')
          : S('.tuva-osasuoritus')
        ...
        osasuoritukset.each(function (i, e) {
          if (
            extractAsText(S(e).find('tr > td.suoritus > button.nimi')).includes(
              nimi
            )
          ) {
            found = S(e)
...
function Opiskeluoikeudet() {
  return {
    ...
    omatTiedotOpiskeluoikeuksienMäärä: function () {
      return S(
        '.oppilaitos-list .oppilaitos-container .opiskeluoikeudet-list button.opiskeluoikeus-button'
      ).length
    },
    opiskeluoikeuksienOtsikot: function () {
      return textsOf(
        S('.opiskeluoikeuksientiedot .opiskeluoikeus h3 .otsikkotiedot')
      )
    },
    omatTiedotOpiskeluoikeuksienOtsikot: function () {
      return textsOf(
        S(
          '.oppilaitos-list .oppilaitos-container .opiskeluoikeudet-list button.opiskeluoikeus-button h3'
        )
      )
    },
    ...
    valitunVälilehdenAlaotsikot: function () {
      return textsOf(
        S(
          '.opiskeluoikeustyypit-nav .selected .opiskeluoikeudet .opiskeluoikeus'
        )
      )
    },
...
function Versiohistoria() {
  ...
  var api = {
    avaa: function () {
      ...
      return wait
        .untilVisible(subElement(elem, 'li.selected'))()
        .then(wait.forAjax)
    },
    sulje: function () {
      if (S('.versiohistoria > .modal').is(':visible')) {
...
function TilaJaVahvistus(elem = findSingle('.tila-vahvistus')) {
  ...
  function merkitseKeskeneräiseksiButton() {
    return elem().find('button.merkitse-kesken')
  }

  var api = {
    merkitseValmiiksiEnabled: function () {
      return (
        merkitseValmiiksiButton().is(':visible') &&
        !merkitseValmiiksiButton().is(':disabled')
      )
    },
    merkitseValmiiksi: seq(
      click(merkitseValmiiksiButton),
      wait.until(function () {
        return isElementVisible(S('.merkitse-valmiiksi-modal'))
      })
    ),
    ...
    lisääVahvistus: function (pvm) {
      ...
      return seq(
        dialogEditor.property('päivä').setValue(pvm),
  ...
  var api = _.merge(
    {
      ...
      selectTutkinto: function (name, selector) {
        ...
        return TutkintoSelector(function () {
          return elem().find(selector)
        }).select(name)
      },
      tutkinto: function () {
        return Page(elem).getInputValue(
          '.koulutusmoduuli input, .tutkinto input'
        )
      },
...
function TutkintoSelector(elem) {
  ...
  var api = {
    select: function (name) {
      return function () {
        return wait
          .until(Page(elem).getInput('input').isVisible)()
...
function OpiskeluoikeusDialog() {
  ...
  return {
    ...
    radioEnabled: function (value) {
      return !findSingle(
        'input[value="koskiopiskeluoikeudentila_' + value + '"]',
        elem()
      )().is(':disabled')
    },
    tilat: function () {
...
function Editor(elem) {
  ...
  var api = {
    ...
    saveChangesAndExpectError: seq(
      click(enabledSaveButton),
      wait.until(KoskiPage().isErrorShown)
    ),
    cancelChanges: seq(
      click(findSingle('#edit-bar .cancel')),
      KoskiPage().verifyNoError
    ),
    isEditable: function () {
  ...
  return _.merge(
    {
      ...
      selectValue: function (value) {
        return function () {
          return Page(elem)
            .setInputValue('.dropdown', value.toString())()
            .then(wait.forAjax)
        }
      },
      setValue: function (value, index) {
        return function () {
          return Page(elem).setInputValue(
            '.dropdown, .editor-input, .autocomplete',
            value,
            index
          )()
        }
      ...
      itemEditor: function (index) {
        return this.propertyBySelector(

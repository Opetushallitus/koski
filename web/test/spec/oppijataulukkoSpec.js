  ...
  describe('Sorttaus', function() {
    describe('nimellä', function() {
      ...
      it('Oletusjärjestys nouseva nimen mukaan', function() {
        expect(page.oppijataulukko.names()).to.deep.equal([ 'Koululainen, Kaisa', 'Lukiolainen, Liisa', 'Oppiaineenkorottaja, Olli', 'Oppija, Oili', 'Toiminta, Tommi', 'Ysiluokkalainen, Ylermi' ])
      })
      it('Laskeva järjestys klikkaamalla', function() {
        return page.oppijataulukko.sortBy('nimi')().then(function() {
          expect(page.oppijataulukko.names()).to.deep.equal([ 'Ysiluokkalainen, Ylermi', 'Toiminta, Tommi', 'Oppija, Oili', 'Oppiaineenkorottaja, Olli', 'Lukiolainen, Liisa', 'Koululainen, Kaisa' ])
        })
      })
    })
    ...
    describe('luokkatiedolla', function() {
      ...
      it('Nouseva järjestys', function() {
        return page.oppijataulukko.sortBy('luokka')().then(function() {
          expect(page.oppijataulukko.data().map(function(row) { return row[7]})).to.deep.equal(['9B', '9C', '9C', '9D'])
        })
      })
      it('Laskeva järjestys', function() {
        return page.oppijataulukko.sortBy('luokka')().then(function() {
          expect(page.oppijataulukko.data().map(function(row) { return row[7]})).to.deep.equal(['9D', '9C', '9C', '9B'])
        })
      })
    })

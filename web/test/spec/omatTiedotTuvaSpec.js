  ...
  describe('TUVAn suorittanut', function () {
    ...
    it('Opiskeluoikeus näytetään listassa', function () {
      expect(
        ...
        'Tutkintokoulutukseen valmentava koulutus (2021—2022, valmistunut)'
      )
    })
    ...
    describe('Kun opiskeluoikeus avataan', function () {
      ...
      it('näytetään suorituksen yleiset tiedot', function () {
        expect(extractAsText(findSingle('.tab.selected'))).to.equal(
          'Tutkintokoulutukseen valmentava koulutus'
        )
        expect(opinnot.opiskeluoikeudet.järjestämislupa()).to.equal(
          'Ammatillisen koulutuksen järjestämislupa (TUVA)'
        )

        opinnot.avaaLisätiedot()
        expect(extractAsText(findSingle('.maksuttomuus .value'))).to.equal(
          '1.8.2021 — Maksuton'
        )

        expect(
          extractAsText(findSingle('.koulutusmoduuli .tunniste'))
        ).to.equal('Tutkintokoulutukseen valmentava koulutus')
        expect(extractAsText(findSingle('.diaarinumero .value'))).to.equal(
          'OPH-1488-2021'
        )
        expect(extractAsText(findSingle('.tila-vahvistus.valmis'))).to.equal(
          'Suoritus valmis Vahvistus : 31.12.2021 Helsinki Reijo Reksi , rehtori'
        )
      })

describe('EPerusteet', function() {
  ...
  describe('Tutkinnon nimi', function() {
    describe('haetaan e-perusteista', function () {
      before(page.openPage, page.oppijaHaku.searchAndSelect('200994-834A'), opinnot.valitseSuoritus(undefined, 'Tieto- ja viestintätekniikan perustutkinto, koulutusvientikokeilu'))
      var koulutus = opinnot.opiskeluoikeusEditor().property('koulutusmoduuli')
      it('toimii', function() {
        expect(koulutus.getText()).to.equal('Koulutus Tieto- ja viestintätekniikan perustutkinto, koulutusvientikokeilu 341101 OPH-1117-2019')
      })
    })
  })
  ...
  describe('Perusteen linkki virkalijan oppija-näkymässä', function() {
    describe('Kun opiskeluoikeuden suoritus on kesken, linkitetään uusimpaan perusteeseen', function () {
      before(
        ...
        wait.until(function() { return isElementVisible(S('.diaarinumero a'))})
      )
    ...
    describe('Kun opiskeluoikeudella on päättymispäivä, linkitetään päättymispäivänä voimassa olleeseen perusteeseen', function () {
      ...
      before(
        ...
        Page().setInputValue('.lisaa-opiskeluoikeusjakso-modal #date-input', paattymispaiva),
        click(findFirst('.lisaa-opiskeluoikeusjakso-modal .tila li:nth-child(5) label')),

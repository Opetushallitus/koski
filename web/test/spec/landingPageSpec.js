describe('Etusivu', function() {
  ...
  describe('Kun virkailija on kirjautunut sisään', function() {
    before(authentication.login(), etusivu.go, wait.until(virkailijaPage.oppijataulukko.isReady))
  ...
  describe('Kun kansalainen on kirjautunut sisään', function() {
    before(authentication.logout, etusivu.openPage, etusivu.login(), wait.until(korhopankki.isReady), korhopankki.login('190751-739W'), wait.until(OmatTiedotPage().isVisible))

    it('näytetään omattiedot sivu', function() {
      expect(extractAsText(S('.user-info .name'))).to.equal('Oili Oppija')
      expect(extractAsText(S('.oppija-content > h2'))).to.equal('Opintosuorituksesi')
    })
  })
})
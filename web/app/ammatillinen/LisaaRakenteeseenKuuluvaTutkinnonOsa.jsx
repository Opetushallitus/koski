...
export const LisääRakenteeseenKuuluvaTutkinnonOsa = ({
  lisättävätTutkinnonOsat,
  addTutkinnonOsa,
  koulutusmoduuliProto,
  placeholder = lisättävätTutkinnonOsat.osanOsa
  ...
  selectedAtom.filter(R.identity).onValue((newItem) => {
    const tutkinnonOsa = modelSetValues(koulutusmoduuliProto(newItem), {
      tunniste: newItem
    })
    addTutkinnonOsa(modelSetTitle(tutkinnonOsa, newItem.title))
  })
  return (
    lisättävätTutkinnonOsat.osat.length > 0 && (

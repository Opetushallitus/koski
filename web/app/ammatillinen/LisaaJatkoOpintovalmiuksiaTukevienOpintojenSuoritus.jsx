...
export const LisääJatkoOpintovalmiuksiaTukevienOpintojenSuoritus = ({
  parentSuoritus,
  suoritusPrototypes,
  groupTitles,
  groupId,
  ...
  const suoritusPrototype = suoritusPrototypes.find(
    isJatkoOpintovalmiuksiaTukevienOpintojenSuoritus
  )

  const add = () =>
    koodistoNimi('tutkinnonosatvalinnanmahdollisuus/1').onValue(
      (suorituksenNimi) => {
        addSuoritus(
          createSuoritusWithTutkinnonOsanRyhmä(
            suoritusPrototype,
  ...
  return alreadyAdded ||
    ...
    <div className="jatkoopintovalmiuksiatukevienopintojensuoritus">
      <a className="add-link" onClick={add}>
        <Text name="Lisää yhteisten tutkinnon osien osa-alueita, lukio-opintoja tai muita jatko-opintovalmiuksia tukevia opintoja" />
      </a>
    </div>
  )
}

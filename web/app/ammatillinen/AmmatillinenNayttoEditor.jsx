...
const NäyttöPopup = ({ model, hasOldData, doneCallback }) => {
  ...
  return (
    <ModalDialog
      className="lisää-näyttö-modal"
      onDismiss={doneCallback}
      onSubmit={() => submitB.push()}
      submitOnEnterKey="false"
      okTextKey={hasOldData ? 'Päivitä' : 'Lisää'}
      validP={validP}
    >
      <h2>
        <Text name="Ammattiosaamisen näyttö" />
      </h2>
                ...
                <tbody>
                  <tr>
                    {modelP.map((m) => [
                      <td>
                        <Editor
                          model={modelLookup(m, 'suorituspaikka.tunniste')}
                        />
                      </td>,
                      <td>
                        <Editor
                          model={modelLookup(m, 'suorituspaikka.kuvaus')}
                        />
                      </td>
                    ])}
                  </tr>
...
const YksittäinenNäyttöEditor = ({ edit, model, popupVisibleA }) => {
  return (
    <div>
      {edit && (
        <a className="edit-value" onClick={() => popupVisibleA.set(true)}>
          <Text name="Muokkaa" />
        </a>
      )}
      {edit && (
        <a
          ...
          onClick={() => resetOptionalModel(model)}
        ></a>
      )}
...
export class AmmatillinenNäyttöEditor extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        {popupVisibleA.map((visible) =>
          visible ? (
            <NäyttöPopup
              edit={edit}
              hasOldData={hasData}
              model={wrappedModel}
              doneCallback={() => popupVisibleA.set(false)}
            />
          ) : null
        )}
        {hasData && (
          <YksittäinenNäyttöEditor
            edit={edit}
            model={model}
            popupVisibleA={popupVisibleA}
          />
        )}
        {edit && !hasData && (
          <a className="add-value" onClick={() => popupVisibleA.set(true)}>
            <Text name="Lisää ammattiosaamisen näyttö" />
          </a>
        )}
      </div>
    )

...
export default ({ suoritus, groupId, suoritusPrototypes, setExpanded, groupTitles }) => {
  ...
  const valtakunnallisetKoulutusmoduulit = valtakunnallisetKoulutusmoduuliPrototypes(suoritusPrototype)
  const paikallinenKoulutusmoduuli = koulutusModuuliprototypes(suoritusPrototype).find(isPaikallinen)

  const koulutusmoduuliProto = selectedItem => selectedItem && isYhteinenTutkinnonOsa(suoritus)
  ...
  const diaarinumero = modelData(suoritus, 'koulutusmoduuli.perusteenDiaarinumero') || modelData(suoritus, 'tutkinto.perusteenDiaarinumero')

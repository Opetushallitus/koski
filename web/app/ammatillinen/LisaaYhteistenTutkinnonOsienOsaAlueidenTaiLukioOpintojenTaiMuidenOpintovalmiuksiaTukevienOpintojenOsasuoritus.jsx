...
import {modelData, modelSetData, modelSetTitle, modelSetValues} from '../editor/EditorModel'
...
export const LisääYhteistenTutkinnonOsienOsaAlueidenTaiLukioOpintojenTaiMuidenOpintovalmiuksiaTukevienOpintojenOsasuoritus = ({parentSuoritus, suoritusPrototypes, setExpanded}) => {
  ...
  return (<div className='yhteistentutkinnonosienosaalueidentailukioopintojentaimuidenopintovalmiuksiatukevienopintojenosasuoritus'>
    <LisääYhteinenTutkinnonOsa {...{suoritusPrototypes, setExpanded}} />
    <LisääLukioOpinto {...{suoritusPrototypes, setExpanded}} />
    <LisääMuuOpintovalmiuksiaTukevaOpinto {...{suoritusPrototypes, setExpanded}} />
  </div>)
}
...
const LisääPaikallinen = ({suoritusPrototype, setExpanded, headerText, className}) => {
  ...
  const hasPeruste = !R.isNil(modelData(koulutusmoduuliPrototype, 'perusteenDiaarinumero'))

  Bacon.combineWith(nameAtom, perusteAtom, (name, peruste) => ({name, peruste})).onValue(np => {
    if (np.name) {
      let koulutusmoduuliModel = modelSetValues(koulutusmoduuliPrototype, { 'kuvaus.fi': { data: np.name}, 'tunniste.nimi.fi': { data: np.name}, 'tunniste.koodiarvo': { data: np.name }})
      if (np.peruste)  {
        koulutusmoduuliModel = modelSetData(koulutusmoduuliModel, np.peruste, 'perusteenDiaarinumero')
      }
  ...
  return (<span className={className}>
    ...
    { ift(addAtom, (<ModalDialog className={`lisaa-${className}-modal`} onDismiss={add} onSubmit={() => add(selectedAtom.get())} okTextKey='Lisää' validP={nameAtom}>
        ...
        { hasPeruste && <Peruste suoritusTyyppiP={Bacon.constant({koodiarvo: 'lukionoppiaineenoppimaara'})} perusteAtom={perusteAtom} /> }
      </ModalDialog>)
    ) }
  </span>)
}
...
const addSuoritus = (setExpanded, suoritusPrototype, koulutusmoduuliPrototype) =>
  pushSuoritus(setExpanded)(createTutkinnonOsa(suoritusPrototype, koulutusmoduuliPrototype))

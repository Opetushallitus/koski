...
class NäytönArvioitsijaEditor extends React.Component {
  render() {
    ...
    return (
      <span>
        {modelTitle(model, 'nimi')}{' '}
        {modelData(model, 'ntm') ? (
          <span>
            {' ('}
            <Text name="näyttötutkintomestari" />
            {')'}
          </span>
        ) : null}
      </span>
...
class OppisopimusEditor extends React.Component {
  render() {
    const { model } = this.props
    return (
      <div className="oppisopimuksellinenjarjestamismuoto">
...
class TutkinnonOsanLisätietoEditor extends React.Component {
  render() {
    const { model } = this.props
    return (
      <div className="ammatillisentutkinnonosanlisatieto">
...
class JärjestämismuotojaksoEditor extends React.Component {
  render() {
    const { model } = this.props
    const propertyName = hasModelProperty(model, 'järjestämismuoto')
      ? 'järjestämismuoto'
      ...
      <div className="jarjestamismuotojakso">
        ...
        <span className="jarjestamismuoto">
          ...
          <PropertiesEditor
            ...
            model={model}
            propertyFilter={(p) =>
              !['alku', 'loppu', propertyName].includes(p.key)
            }
          />
        </span>
      </div>
...
const TyössäoppimisjaksoEditor = ({ model }) => {
  return (
    <div className="tyossaoppimisjakso">
      <PäivämääräväliEditor model={model} /> {modelTitle(model, 'paikkakunta')}
      {', '}
      {modelTitle(model, 'maa')}
      <PropertiesEditor
        model={model}
        propertyFilter={(p) =>
          !['alku', 'loppu', 'paikkakunta', 'maa'].includes(p.key)
        }
      />
    </div>
  )
...
const SisältäväOpiskeluoikeusEditor = ({ model }) => {
    ...
    return (
      <PropertiesEditor
        ...
        getValueEditor={(p, getDefault) => {
          return p.key === 'oppilaitos' ? (
            <OrganisaatioEditor
              model={p.model}
              showAll={true}
              organisaatioTyypit={['OPPILAITOS', 'OPPISOPIMUSTOIMIPISTE']}
            />
          ) : (
            getDefault()
          )
  ...
  return (
    <span>
      <span className="id">
        <Editor model={model} path="oid" />
      </span>
      <span className="oppilaitos">
        {'('}
        <Editor model={model} path="oppilaitos" />
        {')'}
      </span>
    </span>
  )
...
const OsaamisalajaksoEditor = ({ model }) => {
  ...
  const diaarinumero =
    modelData(model.context.suoritus, 'tutkinto.perusteenDiaarinumero') ||
    modelData(model.context.suoritus, 'koulutusmoduuli.perusteenDiaarinumero')
  const osaamisalat = wrappedModel.context.edit
    ? fetchOsaamisalat(diaarinumero)
    : () => Bacon.never()
  ...
  return (
    <span className="osaamisalajakso">
      <span className="property osaamisala">
        <EnumEditor
          model={modelLookup(model, 'osaamisala')}
          fetchAlternatives={osaamisalat}
          displayValue={(option) =>
            option.title +
            (option.data ? ' (' + option.data.koodiarvo + ')' : '')
          }
        />
      </span>
...
const fetchOsaamisalat = (diaari) => () =>
  Http.cachedGet(
    `/koski/api/editor/koodit/osaamisalat/osaamisala/${encodeURIComponent(
      diaari
    )}`
  )

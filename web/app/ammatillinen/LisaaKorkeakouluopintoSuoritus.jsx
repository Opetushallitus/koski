...
import {modelItems, modelLookup, modelSet, modelSetTitle, modelSetValue} from '../editor/EditorModel'
...
export const LisääKorkeakouluopintoSuoritus = ({parentSuoritus, suoritusPrototypes, groupTitles, groupId, addSuoritus}) => {
  const alreadyAdded = modelItems(parentSuoritus, 'osasuoritukset').some(isKorkeakouluOpintosuoritus)
  const suoritusPrototype = suoritusPrototypes.find(isKorkeakouluOpintosuoritus)

  const add = () => koodistoNimi('tutkinnonosatvalinnanmahdollisuus/2').onValue(suorituksenNimi => {
...
export const createSuoritusWithTutkinnonOsanRyhmä = (suoritusPrototype, tutkinnonOsanRyhmänNimi, suorituksenNimi) => {
  const tutkinnonOsanRyhmä = toKoodistoEnumValue('ammatillisentutkinnonosanryhma', AMMATILLISET_TUTKINNON_OSAT, tutkinnonOsanRyhmänNimi)
  const suoritus = modelSetValue(suoritusPrototype, tutkinnonOsanRyhmä, 'tutkinnonOsanRyhmä')
  return modelSet(suoritus, modelSetTitle(modelLookup(suoritus, 'koulutusmoduuli'), suorituksenNimi), 'koulutusmoduuli')
}

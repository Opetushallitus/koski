...
const doUpdate = (secret, expirationDate) =>
  Http.post(`${ApiBaseUrl}/update`, { secret, expirationDate })

export const SuoritusjakoLinkPlaceholder = ({ transition }) => (
  <div
    className={`suoritusjako-link--placeholder${
...
export class SuoritusjakoLink extends React.Component {
  ...
  componentDidMount() {
    const debounced = this.dateChangeBus.toProperty().changes().debounce(500)

    debounced.onValue(() => this.setState({ isDateUpdatePending: true }))

    this.updateBus = debounced.flatMapLatest(
      (date) =>
        date && doUpdate(this.props.suoritusjako.secret, formatISODate(date))
    )
      ...
      <div className="suoritusjako-link">
        <div className="suoritusjako-link__top-container">
          <div className="suoritusjako-link__link-container">
            <span className="suoritusjako-link__timestamp">
              <Text name="Jakolinkki luotu" />{' '}
              {ISO2FinnishDateTime(timestamp).replace(' ', ' klo ')}
            </span>
            <CopyableText
          ...
          <div className="suoritusjako-link__expiration">
            <label htmlFor={labelId}>
              <Text name="Linkin voimassaoloaika päättyy" />
            </label>
            <div
              style={{ display: 'inline' }}
              className={isDateUpdatePending ? 'ajax-indicator-right' : ''}
            >
              <DateInput
                ...
                isAllowedDate={(d) =>
                  SuoritusjakoLink.isDateInFuture(d) &&
                  SuoritusjakoLink.isDateWithinYear(d)
                }
        ...
        <div className="suoritusjako-link__bottom-container">
          <div className="suoritusjako-link__preview">
            ...
            >
              <Text name="Katso, miltä suoritusote näyttää selaimessa" />
            </a>
          </div>
          <div className="suoritusjako-link__remove">
            <button
              className={`text-button-small${
                isDateUpdatePending ? '--disabled' : ''
              }`}
              onClick={this.confirmDelete.bind(this)}
              disabled={isDateUpdatePending}
            >

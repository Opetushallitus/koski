...
export const AikuistenPerusopetuksenKurssitEditor = ({ model }) => {
  ...
  const kurssinSuoritusProto = newOsasuoritusProto(
    model,
    'aikuistenperusopetuksenkurssinsuoritus'
  )

  const showUusiAlkuvaiheenKurssiAtom = Atom(false)
  const alkuvaiheenKurssinSuoritusProto = newOsasuoritusProto(
    model,
      ...
      {model.context.edit && osasuoritusCountOk(osasuoritukset) && (
        <React.Fragment>
          <UusiKurssi
            name={'Lisää kurssi'}
            model={model}
            classname={'uusi-kurssi'}
            showUusiKurssiAtom={showUusiKurssiAtom}
            kurssinSuoritusProto={kurssinSuoritusProto}
            customAlternativesCompletionFn={(oppiaine, kurssiProtos) =>
              customAlternativesFn(
                oppiaine,
                kurssiProtos,
          ...
          />
          {fromBacon(
            oppiaineMyösAlkuvaiheessaP(model).map(
              (myösAlkuvaiheessa) =>
                myösAlkuvaiheessa && (
                  <UusiKurssi
                    name={'Lisää alkuvaiheen kurssi'}
                    model={model}
                    classname={'uusi-alkuvaiheen-kurssi'}
...
const KurssitOtsikko = ({ model }) => {
  const suorituksiaTehty =
    modelItems(model, 'osasuoritukset').filter(arvioituTaiVahvistettu).length >
    0
  return (
    (model.context.edit || suorituksiaTehty) && (
      <h5>
        <Text name="Kurssit" />
      </h5>
    )
...
const Kurssit = ({ model }) => (
  <ul>
    {modelItems(model, 'osasuoritukset').map((kurssi, index) => (
      <KurssiEditor key={index} kurssi={kurssi} />
    ))}
  </ul>
)
  ...
  <span className={classname}>
    ...
    {ift(
      ...
      <UusiKurssiPopup
        ...
        resultCallback={(kurssi) =>
          lisääKurssi(kurssi, model, showUusiKurssiAtom, kurssinSuoritusProto)
        }
...
const oppiaineMyösAlkuvaiheessaP = (model) => {
  const oppiaineenKoodiarvo = modelData(
    model,
    'koulutusmoduuli.tunniste.koodiarvo'
  )
  return koodistoValues('aikuistenperusopetuksenalkuvaiheenoppiaineet').map(
    (mahdollisetAlkuvaiheenOppiaineet) =>
      mahdollisetAlkuvaiheenOppiaineet.find(
        (o) => o.koodiarvo === oppiaineenKoodiarvo
      )
  )
}
...
const customAlternativesFn = (oppiaine, kurssiPrototypes, oppiaineKoodisto) => {
  ...
  const oppimaaraKoodisto =
    modelData(oppiaine, 'kieli.koodistoUri') ||
    modelData(oppiaine, 'oppimäärä.koodistoUri')
  const oppimaaraKoodiarvo =
    modelData(oppiaine, 'kieli.koodiarvo') ||
    modelData(oppiaine, 'oppimäärä.koodiarvo')
  ...
  const fetchAlternatives = (model) => {
    ...
    const kurssiKoodistot =
      koodistoAlternativesPath && R.last(koodistoAlternativesPath.split('/'))

    const loc = parseLocation(
      `/koski/api/editor/koodit/${oppiaineKoodisto}/${oppiaineKoodiarvo}/kurssit/${kurssiKoodistot}`
    ).addQueryParams({
      oppimaaraKoodisto,
      oppimaaraKoodiarvo,
      oppimaaraDiaarinumero
    })
  ...
  return Bacon.combineAsArray(kurssiPrototypes.map(fetchAlternatives))
    .last()
    .map(R.unnest)
}

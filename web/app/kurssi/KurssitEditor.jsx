...
import {isLukio2019ModuuliTaiOpintojakso, koulutusModuuliprototypes, isPaikallinen, isLukio2019Oppiaine} from '../suoritus/Koulutusmoduuli'
...
export const KurssitEditor = ({model, customTitle, customAlternativesCompletionFn, customKurssitSortFn}) => {
  ...
  const lukionOppiaine = isLukio2019Oppiaine(modelLookup(model, 'koulutusmoduuli'))

  const kurssiPrototypes = (lukionOppiaine && !isPreIb) ?
    osasuoritusProtos.map(osasuoritusProto => koulutusModuuliprototypes(osasuoritusProto)).flat() :
        ...
        <a onClick={() => showUusiKurssiAtom.set(true)}><Text name={`Lisää ${customTitle || 'kurssi'}`}/></a>
        {
          ift(showUusiKurssiAtom,
            <UusiKurssiPopup
              ...
              resultCallback={(k) => lisääKurssi(k, model, showUusiKurssiAtom, sopivaOsasuorituksenPrototyyppi(k))}

...
export const UusiKurssiDropdown = ({
  ...
  const käytössäolevatKoodiarvot = suoritukset.map(
    (s) => modelData(s, 'koulutusmoduuli.tunniste').koodiarvo
  )
  const kurssiKoodit = customAlternativesCompletionFn || fetchKurssiKoodit
  const valtakunnallisetKurssit = kurssiKoodit(
    oppiaine,
    valtakunnallisetKurssiProtot
  )
  const paikallisetKurssit = Atom([])
  const setPaikallisetKurssit = (kurssit) => paikallisetKurssit.set(kurssit)

  if (paikallinenKurssiProto) {
    getOrganizationalPreferences(
      organisaatioOid,
      paikallinenKurssiProto.value.classes[0]
    ).onValue(setPaikallisetKurssit)
  }

  const displayValue = (kurssi) => {
    if (isDiaKurssi(kurssi)) {
      return modelTitle(kurssi, 'tunniste')
    } else {
      return (
        modelData(kurssi, 'tunniste.koodiarvo') +
        ' ' +
        modelTitle(kurssi, 'tunniste')
      )
    }
  }
  const kaikkiKurssit = Bacon.combineWith(
    paikallisetKurssit,
    valtakunnallisetKurssit,
    (x, y) => x.concat(y)
  )
    .map((kurssit) =>
      kurssit.filter(
        (kurssi) =>
          isPaikallinen(kurssi) ||
  ...
  const poistaPaikallinenKurssi = (kurssi) => {
  ...
  return (
    <div className={'uusi-kurssi'}>
      {elementWithLoadingIndicator(
        kaikkiKurssit
          .map('.length')
          .map((length) =>
            length || paikallinenKurssiProto ? (
              <DropDown
                ...
                removeText={t(
                  'Poista paikallinen kurssi. Poistaminen ei vaikuta olemassa oleviin suorituksiin.'
                )}
              />
            ) : null
          )
...
const fetchKurssiKoodit = (oppiaine, kurssiPrototypes) => {
  ...
  const alternativesForField = (model) => {
    ...
    const koodistot = kurssiKoodistot.split(',').filter((koodisto) => {
      if (
        (koodisto === 'lukioonvalmistavankoulutuksenmoduulit2019' &&
          oppimaaraDiaarinumero === '56/011/2015') ||
        (koodisto === 'lukioonvalmistavankoulutuksenkurssit2015' &&
    ...
    const loc = parseLocation(
      `/koski/api/editor/koodit/${oppiaineKoodisto}/${oppiaineKoodiarvo}/kurssit/${queryKoodistot}`
    ).addQueryParams({
      oppimaaraKoodisto,
      oppimaaraKoodiarvo,

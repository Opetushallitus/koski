...
const OmatTiedotSuoritustaulukko = ({
  suorituksetModel,
  nested,
  parentSuoritus: parentSuoritusProp
}) => {
  ...
  return (
    <div className="omattiedot-suoritus-taulukko">
      {groupsP.map((groups) =>
        flatMapArray(groups.groupIds, (groupId, i) => (
          <SuoritusGroup
            key={`group-${i}`}
            groups={groups}
            groupId={groupId}
...
const arvosanaColumn = (parentSuoritus) =>
  isYlioppilastutkinto(parentSuoritus)
...
const YtrArvosanaColumn = (parentSuoritus) => {
  ...
  const koesuorituksetP = suoritusjako
    ? Bacon.constant([])
    : Http.post(
        '/koski/api/ytrkoesuoritukset/' + parentSuoritus.context.oppijaOid,
        { huollettava },
        { errorMapper: () => undefined }
      )

  return {
    shouldShow: ({ suoritukset, context }) =>
    ...
    renderData: ({ model }) => {
      ...
      return koesuorituksetP
        .map(
          (kokeet) =>
            kokeet &&
            kokeet.find((koe) => koe.period === period && koe.examId === examId)
        )
        .map((koe) => {
          return (
            <React.Fragment key="data">
              <td key="arvosana" className="arvosana ylioppilas">
                <ArvosanaEditor model={model} />
              </td>
              <td key="koesuoritus" className="koesuoritus">
                <KoesuoritusLink
                  copyOfExamPaper={koe && koe.copyOfExamPaper}
                  kokeenNimi={modelData(model, 'koulutusmoduuli.tunniste.nimi')}
                  parentSuoritus={parentSuoritus}
                />
              </td>
            </React.Fragment>
...
const KoesuoritusLink = ({ copyOfExamPaper, kokeenNimi, parentSuoritus }) =>
  copyOfExamPaper ? (
    <a
      ...
      target="_blank"
      href={`/koski/koesuoritus/${copyOfExamPaper}${
        parentSuoritus.context.huollettava
          ? '?huollettava=' + parentSuoritus.context.oppijaOid
          : ''
      }`}
      rel="noopener noreferrer"
    >
      <Text
        className="show-koesuoritus-text"
        name="Näytä koesuoritus"
        aria-label={t(kokeenNimi) + '. ' + t('Näytä koesuoritus')}
      />
    </a>
  ) : null
      ...
      <table className={nested ? 'nested' : ''}>
        <thead>
          <tr className={nested ? 'nested-header' : ''}>
            {columns.map((column) =>
              column.renderHeader({ groupTitles, groupId })
            )}
          </tr>
        </thead>
        <tbody>
      ...
      {!nested &&
        ...
        !ylioppilastutkinto && (
          <YhteensäSuoritettu
            ...
            laajuusP={fetchLaajuudet(parentSuoritus, groups.groupIds).map(
              (l) => l[groupId]
            )}
            laajuusYksikkö={laajuusYksikkö}
          />
  ...
  render() {
    ...
    return [
      ...
      expanded && hasProperties && (
        <tr key="properties" className="details">
          <td colSpan="4">
            <PropertiesEditor
              model={model}
      ...
      expanded && showOsasuoritukset && (
        <tr key="osasuoritukset" className="osasuoritukset">
          <td colSpan="4">
...
const SuoritusColumn = {
  shouldShow: () => true,
  renderHeader: ({ groupTitles, groupId }) => (
    <th key="suoritus" className="tutkinnon-osan-ryhma" scope="col">
      {groupTitles[groupId]}
    </th>
  ),
  renderData: ({ model, onExpand, expandable, expanded }) => {
    ...
    return (
      <td key="suoritus" className="suoritus">
        {expandable && (
          <div className="expanded-indicator" aria-hidden={true}>
            {expanded ? '-' : '+'}
          </div>
        )}
        {expandable ? (

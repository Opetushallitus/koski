...
export class OsaaminenTunnustettuEditor extends React.Component {
  ...
  render() {
    ...
    const seliteModel = modelLookup(model, 'selite')

    return (
      <div>
        {hasData ? (
          <span>
            <span className="selite">
              <Editor model={seliteModel} autoFocus={true} />
            </span>
            <a className="remove-value" onClick={removeValue(model)}></a>
            <PropertiesEditor
              model={model}
              propertyFilter={(p) => ['rahoituksenPiirissä'].includes(p.key)}
            />
          </span>
        ) : (
          <span>
            <a className="add-value" onClick={addValue(model)}>
              <Text name="Lisää osaamisen tunnustaminen" />
            </a>
          </span>
        )}
      </div>

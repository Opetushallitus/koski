...
export const PerusteEditor = ({model}) => {
  if (!model.context.edit) {
    ...
    const paattymispaiva = modelData(model.context.opiskeluoikeus).päättymispäivä
    return peruste ? fromBacon(perusteLinkki(peruste, <StringEditor model={model}/>, paattymispaiva)) : <StringEditor model={model}/>
  }
...
const perusteLinkki = (peruste, perusteEditor, paattymispaiva) => {
  ...
  const paattymisquery = paattymispaiva ? '&päättymispäivä='+paattymispaiva : ''
  return Http.cachedGet(`/koski/api/tutkinnonperusteet/peruste/${encodeURIComponent(peruste)}/linkki?lang=${encodeURIComponent(lang)}${paattymisquery}`, map404).map('.url').map(linkki =>

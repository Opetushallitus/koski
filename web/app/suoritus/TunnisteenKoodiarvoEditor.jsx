...
export const TunnisteenKoodiarvoEditor = ({model}) => {
  ...
  const updateTunniste = koulutus => pushModelValue(tunnisteModel, {data: koulutus})
  const automaticUpdatePossible = koulutukset => koulutukset.length === 1 && modelData(tunnisteModel, 'koodiarvo') !== koulutukset[0].koodiarvo

  koulutuksetDiaarinumerolleP.onValue(koulutukset => model.context.edit && automaticUpdatePossible(koulutukset) && updateTunniste(koulutukset[0]))

  return (
    <span>
       {
         koulutuksetDiaarinumerolleP.map(koulutukset => koulutukset.length > 1 && model.context.edit
           ? <TutkintoKoodiDropdown options={koulutukset} selected={modelData(tunnisteModel)} onSelectionChanged={updateTunniste}/>
...
const fetchKoulutuksetDiaarille = diaari => Http.cachedGet(`/koski/api/editor/koodit/koulutukset/koulutus/${encodeURIComponent(diaari)}`)

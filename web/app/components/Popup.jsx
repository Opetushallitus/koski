...
export class Popup extends React.Component {
  ...
  render() {
    const {showStateAtom, dismissedStateValue = false, inline, children, closeTitle} = this.props
    const dismiss = () => showStateAtom.set(dismissedStateValue)
    const closeButtonLabel = closeTitle ? `${t('Sulje')} ${t(closeTitle)}` : t('Sulje')

    return (
      <div className={`popup${inline ? ' popup--inline' : ''}`} ref={elem => this.popupContainer = elem} tabIndex='-1'>

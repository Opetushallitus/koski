...
const copyToClipboard = (message, copyState) => () => {
  ...
  try {
    ...
    Bacon.later(ConfirmationDuration, CopyState.PENDING).onValue((v) =>
      copyState.set(v)
    )
  } catch (e) {
...
export const CopyableText = ({
  ...
  const buttonState = copyState.map((state) => {
    switch (state) {
      case CopyState.PENDING:
        return { isDisabled: false, style: '', text: buttonText }
      case CopyState.SUCCESS:
        return {
          isDisabled: true,
          style: '--success',
          text: buttonTextSuccess
        }
      case CopyState.ERROR:
        return { isDisabled: true, style: '--error', text: buttonTextFailure }
    }
  })
  ...
  return (
    <div className={`copyable-text${className ? ' ' + className : ''}`}>
      ...
      {buttonState.map(({ isDisabled, style, text }) => (
        <button
          className={
            style ? `koski-button koski-button${style}` : 'koski-button'
          }
          disabled={isDisabled}
          style={height && { height }}

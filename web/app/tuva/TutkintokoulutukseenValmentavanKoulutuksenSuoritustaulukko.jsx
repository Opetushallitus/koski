...
export class TutkintokoulutukseenValmentavanKoulutuksenSuoritustaulukko extends React.Component {
  render() {
    ...
    const paikallinenOsaSuoritusTitle =
      nestedLevel === 0
        ? title
        : t('Valinnaisen koulutuksen osan paikallinen osasuoritus')
        ...
        <table>
          ...
          <tbody className="taulukko-headers">
            <tr>
              {columns.map((column) =>
                column.renderHeader({
                  laajuusYksikkö,
                  suoritusTitle: paikallinenOsaSuoritusTitle
                })
              )}
            </tr>
          </tbody>
          {suoritukset.map((suoritus, i) => (
            <TutkintokoulutukseenValmentavanKoulutuksenOsasuoritusEditor
              baret-lift
              key={i}
              model={suoritus}
          ...
          {context.edit && (
            <SingleColumnRowTable colSpan={4}>
              <UusiTutkintokoulutukseenValmentavanKoulutuksenOsasuoritus

...
export const UusiTutkintokoulutukseenValmentavanKoulutuksenOsasuoritus = ({
  suoritusPrototypes,
  setExpanded,
  suoritukset
}) => {
  const findSuoritus = (tyyppi) =>
    suoritukset.find((s) => s.value.classes.includes(tyyppi))
  const findSuoritusByKoulutusmoduuli = (tyyppi) => {
    return suoritukset.find((s) =>
      s.value.properties.find(
        (p) =>
          p.key === 'koulutusmoduuli' &&
          p.model.value.classes.find((c) => c === tyyppi)
      )
    )
  }
  const findSuoritusPrototyyppi = (tyyppi) =>
    suoritusPrototypes.find((s) => s.value.classes.includes(tyyppi))

  const tuvaOpiskeluJaUrasuunnittelu =
    !findSuoritusByKoulutusmoduuli(
  ...
  const tuvaTyöelämäTaidot =
    !findSuoritusByKoulutusmoduuli(
      'tutkintokoulutukseenvalmentavattyoelamataidotjatyopaikallatapahtuvaoppiminen'
    ) &&
    findSuoritusPrototyyppi(
      'tutkintokoulutukseenvalmentavakoulutuksenmuunosansuoritus'
    )
  ...
  const tuvaOsaSuorituksenOsaSuoritusVapaavalintaiset = findSuoritusPrototyyppi(
  ...
  return (
    <>
      {tuvaOpiskeluJaUrasuunnittelu && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaOpiskeluJaUrasuunnittelu}
          setExpanded={setExpanded}
          lisääText={
            'Lisää opiskelu- ja urasuunnittelutaitojen koulutuksen osa'
          }
      ...
      {tuvaOsaSuoritusPerustaitojenVahvistaminen && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaOsaSuoritusPerustaitojenVahvistaminen}
          setExpanded={setExpanded}
      ...
      {tuvaAmmatillisenKoulutuksenOpinnot && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaAmmatillisenKoulutuksenOpinnot}
          setExpanded={setExpanded}
          lisääText={
            'Lisää ammatillisen koulutuksen opintojen ja niihin valmistautumisen koulutuksen osa'
          }
          koulutusModuuliTyyppi={
            'tutkintokoulutukseenvalmentavatammatillisenkoulutuksenopinnot'
          }
      ...
      {tuvaLukioKoulutuksenOpinnot && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaLukioKoulutuksenOpinnot}
          setExpanded={setExpanded}
          lisääText={
            'Lisää lukiokoulutuksen opintojen ja niihin valmistautumisen koulutuksen osa'
          }
          koulutusModuuliTyyppi={
            'tutkintokoulutukseenvalmentavatlukiokoulutuksenopinnot'
          }
      ...
      {tuvaTyöelämäTaidot && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaTyöelämäTaidot}
          setExpanded={setExpanded}
          lisääText={
            'Lisää työelämätaitojen ja työpaikalla tapahtuvan oppimisen koulutuksen osa'
          }
          koulutusModuuliTyyppi={
            'tutkintokoulutukseenvalmentavattyoelamataidotjatyopaikallatapahtuvaoppiminen'
          }
      ...
      {tuvaArjenJaYhteiskunnallisenOsallisuudenTaidot && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaArjenJaYhteiskunnallisenOsallisuudenTaidot}
          setExpanded={setExpanded}
          lisääText={
            'Lisää arjen ja yhteiskunnallisen osallisuuden taitojen koulutuksen osa'
          }
          koulutusModuuliTyyppi={
            'tutkintokoulutukseenvalmentavatarjenjayhteiskunnallisenosallisuudentaidot'
          }
      ...
      {tuvaOsaSuoritusVapaavalintaiset && (
        <LisääOsasuoritus
          suoritusPrototype={tuvaOsaSuoritusVapaavalintaiset}
          setExpanded={setExpanded}
      ...
      {tuvaOsaSuorituksenOsaSuoritusVapaavalintaiset && (
        <LisääPaikallinen
          suoritusPrototype={tuvaOsaSuorituksenOsaSuoritusVapaavalintaiset}
          setExpanded={setExpanded}
...
const LisääOsasuoritus = ({
  ...
  const valinnainenOsasuoritus =
    koulutusModuuliTyyppi ===
    'tutkintokoulutukseenvalmentavankoulutuksenvalinnaisenkoulutusosa'

  const koulutusmoduuliPrototype = valinnainenOsasuoritus
    ? koulutusModuuliprototypes(suoritusPrototype)[0]
    : koulutusModuuliprototypes(suoritusPrototype).find((s) =>
        s.value.classes.includes(koulutusModuuliTyyppi)
      )
  ...
  return (
    <div className={'lisaa-uusi-suoritus'}>
      <span className={`lisaa-uusi-tuva-osasuoritus ${className}`}>
        <a className="add-link" onClick={() => addNewSuoritus()}>
          <Text name={lisääText} />
        </a>
      </span>
    </div>
  ...
  return (
    <div className={'lisaa-uusi-suoritus paikallinen'}>
      <span className="lisaa-paikallinen-suoritus">
        <a className="add-link" onClick={() => showModal.set(true)}>
          <Text name={lisääText} />
        </a>
        {ift(
          showModal,
          <ModalDialog
            className="lisaa-paikallinen-tuva-suoritus-modal"

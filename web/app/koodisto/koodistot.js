...
export const tutkinnonOsanRyhmät = (diaarinumero, suoritustapa) => suoritustapa
  ? Http.cachedGet(`/koski/api/tutkinnonperusteet/tutkinnonosat/ryhmat/${encodeURIComponent(diaarinumero)}/${suoritustapa.koodiarvo}`)
    .map(json => R.fromPairs(json.map(koodi => [koodi.koodiarvo, (koodi.nimi[lang] || koodi.nimi.fi)])))
  : Bacon.constant([])

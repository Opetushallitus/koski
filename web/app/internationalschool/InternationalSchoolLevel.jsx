...
export const InternationalSchoolLevel = ({ model }) => {
  if (
    modelData(model.context.opiskeluoikeus, 'tyyppi.koodiarvo') !==
    'internationalschool'
  )
    return null

  return model.context.edit &&
    suorituksenTyyppi(model.context.suoritus) ===
      'internationalschooldiplomavuosiluokka' ? (
    <DiplomaTypeEditor model={model} />
  ) : (
    <span className="international-school-level">{` (${internationalSchoolLevel(
      model
    )})`}</span>
  )
}
...
const internationalSchoolLevel = (model) => {
  switch (suorituksenTyyppi(model.context.suoritus)) {
    ...
    case 'internationalschooldiplomavuosiluokka':
      return `${
        modelData(model, 'diplomaType.koodiarvo') === 'ish' ? 'ISH' : 'IB'
      } Diploma`
  }
}
...
const DiplomaTypeEditor = ({ model }) => {
  ...
  diplomaTypeAtom
    .filter((x) => !R.equals(initialDiplomaType, x))
  ...
  return (
    <KoodistoDropdown
      className="internationalschooldiplomatype"
      options={koodistoValues('internationalschooldiplomatype')}
      selected={diplomaTypeAtom}
    />
  )
}

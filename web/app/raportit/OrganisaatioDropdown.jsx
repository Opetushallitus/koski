...
const filterOrgTree = (query, orgs) => {
  return orgs.map(filterOrg(query)).filter((o) => o.match)
}
...
const filterOrg = (query) => (org) => {
  ...
  return {
    ...org,
    children: matches ? org.children : filteredChildren,
    match: filteredChildren.length > 0 || matches
  }
}

const flattenOrgTree = (orgs) =>
  orgs.flatMap((org) => [org, ...flattenOrgTree(org.children)])
...
export const OrganisaatioDropdown = ({
  ...
  const displayP = Bacon.combineWith(
    openAtom,
    filterAtom,
    selectedP,
  ...
  const filteredOrgsP = Bacon.combineWith(
    filterAtom,
    organisaatiotP,
    (filter, orgs) => {
      const query = filter && filter.toLowerCase()
      return filter ? filterOrgTree(query, orgs) : orgs
    }
  )

  Bacon.combineAsArray(filteredOrgsP, selectedP)
    ...
    .forEach(([filteredOrgs, selected, keySelect]) => {
      ...
      const newIndex =
        flatOrgList.findIndex((org) => org.oid === selected.oid) + keySelect
...
const Options = ({ organisaatiot, selected, onSelect, isOpen, isChild }) => (
  <ul
    className={isChild ? 'child-options' : isOpen ? 'options open' : 'options'}
  >
    {organisaatiot.map((org) => {
      const clickHandler = org.selectable ? () => onSelect(org) : null

      return (
        <li key={org.oid} className="option">
          <span

...
export default ({ user }) => {
  ...
  const classNameP = editAtom.map(
    (isEdit) => 'localization-edit-bar' + (isEdit ? ' visible' : '')
  )
  const editAll = Atom(false)
  return (
    <div>
      {editAtom.and(editAll).map((e) => e && <EditAllTexts />)}
      <div className={classNameP}>
        {'Lokalisoitujen tekstien muokkaus'}
        <a className="cancel" onClick={cancelChanges}>
          {'Peruuta'}
        </a>
        <button
          className="koski-button"
          disabled={hasChanges.not()}
          onClick={saveChanges}
        >
          {'Tallenna'}
        </button>
        ...
        <label className="show-all">
          ...
          <input
            type="checkbox"
            checked={editAll}
            onChange={(e) => editAll.set(e.target.checked)}
          />
        </label>
      </div>
    </div>
...
const EditAllTexts = () => {
  return (
    <div className="localization-edit-all">
      <table>
        <thead>
          <tr>
            <th>{'Suomeksi'}</th>
        ...
        <tbody>
          {' '}
          {R.keys(window.koskiLocalizationMap)

...
export default ({suoritusAtom, oppilaitosAtom, suorituskieliAtom}) => {
  ...
  const suoritustyypitP = koodistoValues('suorituksentyyppi/perusopetuksenoppimaara,nuortenperusopetuksenoppiaineenoppimaara')
  ...
  const suoritusPrototypeP = suoritustyyppiAtom.map('.koodiarvo').flatMap(suorituksenTyyppi => {
    if (suorituksenTyyppi == 'nuortenperusopetuksenoppiaineenoppimaara') {
      return Http.cachedGet('/koski/api/editor/prototype/fi.oph.koski.schema.NuortenPerusopetuksenOppiaineenOppimääränSuoritus')
    }
  }).toProperty()
  ...
  return (<span>
    ...
    {
      suoritustyyppiAtom.map( tyyppi => koodiarvoMatch('nuortenperusopetuksenoppiaineenoppimaara')(tyyppi)

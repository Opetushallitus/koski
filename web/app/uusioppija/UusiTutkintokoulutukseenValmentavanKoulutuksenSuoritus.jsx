...
export const UusiTutkintokoulutukseenValmentavanKoulutuksenSuoritus = ({
  suoritusAtom,
  oppilaitosAtom,
  suorituskieliAtom
}) => {
  ...
  return (
    <div>
      <Peruste
        {...{ suoritusTyyppiP: suoritusP.map('.tyyppi'), perusteAtom }}
      />
    </div>
  )
}
...
export const getTuvaLisätiedot = (tuvaJärjestämislupa) => {
  ...
  switch (tuvaJärjestämislupa.koodiarvo) {
    case 'ammatillinen':
      return {
        lisätiedot: { koulutusvienti: false, pidennettyPäättymispäivä: false }
      }
    case 'lukio':
      return { lisätiedot: { pidennettyPäättymispäivä: false } }
    case 'perusopetus':
      return { lisätiedot: { pidennettyPäättymispäivä: false } }
    default:
      return {}

...
export const Opiskeluoikeus = ({opiskeluoikeusAtom}) => {
  ...
  const opiskeluoikeustyypitP = oppilaitosAtom.flatMapLatest((oppilaitos) => (oppilaitos ? Http.cachedGet(`/koski/api/oppilaitos/opiskeluoikeustyypit/${oppilaitos.oid}`) : []))
    .toProperty().map(tyypit => tyypit.filter(tyyppi => ['ammatillinenkoulutus', 'perusopetus'].includes(tyyppi)))

  opiskeluoikeustyypitP.changes().onValue((tyypit) => {
    opiskeluoikeudenTyyppiAtom.set(tyypit.length == 1 ? tyypit[0] : undefined)
  })

  const suorituksetP = Bacon.combineWith(opiskeluoikeudenTyyppiAtom, tutkintoAtom, oppilaitosAtom, (tyyppi, tutkinto, oppilaitos) => {
    if (tutkinto && oppilaitos && tyyppi == 'ammatillinenkoulutus') {
      return [{
          koulutusmoduuli: {
            tunniste: {
    ...
    } else if (oppilaitos && tyyppi == 'perusopetus') {
      return [{
        koulutusmoduuli: {
          tunniste: {
            koodiarvo: '201101',
  ...
  const opiskeluoikeusP = Bacon.combineWith(dateAtom, oppilaitosAtom, opiskeluoikeudenTyyppiAtom, suorituksetAtom, tilaAtom, (date, oppilaitos, tyyppi, suoritukset, tila) => { return date && oppilaitos && tyyppi && suoritukset && tila && {
      tyyppi: {
  ...
  return (<div>
    {
      Bacon.combineWith(oppilaitosAtom, tutkintoAtom, opiskeluoikeudenTyyppiAtom, (oppilaitos, tutkinto, opiskeluoikeudenTyyppi) => <div>

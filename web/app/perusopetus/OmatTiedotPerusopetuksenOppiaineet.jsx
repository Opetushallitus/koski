...
const ArvosteluInfo = ({ model }) => (
  <section className="arvostelu">
    <h4>
      <Text
        name={
          (isToimintaAlueittain(model) ? 'Toiminta-alueiden' : 'Oppiaineiden') +
          ' arvosanat'
        }
      />
    </h4>
...
const KäyttäytymisenArvio = ({ model }) => {
  ...
  const shouldShow =
    käyttäytymisenArvioModel && modelData(käyttäytymisenArvioModel)
  return shouldShow ? (
    <section className="kayttaytymisen-arvio">
      ...
      <PropertiesEditor
        model={käyttäytymisenArvioModel}
...
const Oppiainetaulukko = ({ model, suoritukset, showLaajuus }) => {
  ...
  const filteredSuoritukset = isVuosiluokkaTaiPerusopetuksenOppimäärä(model)
    ? suoritukset
    : suoritukset.filter(
        (s) => arvioituTaiVahvistettu(s) || osasuoritukset(s).length
      )

  const valmistavanOpetuksenSuoritukset = filteredSuoritukset.filter(
    (s) => !isNuortenPerusopetuksenOppiaineenSuoritusValmistavassaOpetuksessa(s)
  )

  const muutSuoritukset = filteredSuoritukset.filter((s) =>
    isNuortenPerusopetuksenOppiaineenSuoritusValmistavassaOpetuksessa(s)
  )

  const perusopetukseenValmistavanListaus = (
    <tbody>
      <tr className="perusopetus-valmistava-oppiane-tyyppi">
        <Text name="Perusopetuksen valmistavan opetuksen opinnot" />
      </tr>
      <th className="oppiaine" scope="col">
        <Text
          name={isToimintaAlueittain(model) ? 'Toiminta-alue' : 'Oppiaine'}
        />
      </th>
      {showArvosana && (
        <th className="arvosana" scope="col" style={{ textAlign: 'center' }}>
          <Text name="Arvosana" />
        </th>
      )}
      ...
      <tr className="perusopetus-valmistava-oppiane-tyyppi">
        <Text name="Perusopetuksen oppimäärään sisältyvät opinnot" />
      </tr>
      <th className="oppiaine" scope="col">
        <Text
          name={isToimintaAlueittain(model) ? 'Toiminta-alue' : 'Oppiaine'}
...
class OppiaineRow extends React.Component {
  ...
  render() {
    ...
    const oppiaineRowClassName =
      'oppiaine-row' +
      (expandable ? ' expandable' : '') +
      (expandable && expanded ? ' expanded' : '')

    const oppiaineClassName = `oppiaine ${
      isPaikallinen(oppiaine) ? 'paikallinen' : ''
    }`

    return [
      <tr
        className={oppiaineRowClassName}
        key="oppiaine-row"
      ...
      showKurssitRow &&
        (isMobile ? (
          expandable &&
          expanded && (
            <tr className="kurssit-row-mobile" key="kurssit-row">
              <KurssitListMobile oppiaine={model} />
            </tr>
          )
        ) : (
...
const oppiaineTitle = (aine) => {
  ...
  return kieliaine
    ? `${oppiaineenNimi}, ${kielenNimi.toLowerCase()}`
    : oppiaineenNimi
}

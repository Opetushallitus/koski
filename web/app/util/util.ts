...
export const scrollElementBottomVisible = (elem: HTMLElement) => {
  if (elem) {
    const elementTopPositionOnScreen = elem.getBoundingClientRect().y
    const elementBottomPositionOnScreen =
      elementTopPositionOnScreen + elem.getBoundingClientRect().height
    const marginBottom = window.innerHeight - elementBottomPositionOnScreen - 20
    const marginTop = elementTopPositionOnScreen - 20
    const toScroll = -marginBottom
...
export const debugObservables = (
  observables: Record<string, Bacon.Observable<any>>
) => {

...
export const kelaVirkailijaP = (path) => {
  const loadingOppijaStream = Bacon.once(fetchKelaOppijaJson(path))
    .flatMap((oppijaP) =>
      oppijaP.map((json) => (json ? <KelaOpinnot oppija={json} /> : <Empty />))
    )
...
const fetchKelaOppijaJson = (path) => {
  if (path.includes('koski/kela/versiohistoria/')) {
    const [oppijaOid, opiskeluoikeusOid, versio] = R.takeLast(
      3,
      path.split('/')
    )
    ...
    return hetu
      ? kaikkiUusimmatOpiskeluoikeudet(hetu)
      : Bacon.constant(undefined)
  }
}
...
const KelaOpinnot = ({ oppija }) => {
  const opiskeluoikeudet = sortOpiskeluoikeudetJaSuoritukset(

...
const rowInTable = ({ key, value, path }, index) => {
  const ReactComponentForDisplayingValue =
    spesificComponent(key, value) || componentForType[R.type(value)]

  return (
    <tr className={pathToClassnames(path)} key={index}>
      <td className="label">{t(formatLabel(key))}</td>
      <td className="value">
    ...
    <ul>
      {value.map((val, i) => {
        const ReactComponent =
          spesificComponent('key', val) || componentForType[R.type(val)]
        return (
          <li>
...
const ObjectView = ({ value, path }) => {
  ...
  const optionalAikajaksoInline = isAikajakso(value) && (
...
const Laajuus = ({ value }) => (
  <>
    <span>{value.arvo}</span> <Koodistoviite value={value.yksikkö} />
  </>
)
      ...
      {jaksot.map((item, i) => {
        return (
          <li key={i}>
            <div
              className={
                'opiskeluoikeusjakso' + (i === activeIndex ? ' active' : '')
              }
            >
              ...
              <label className="rahoitusmuoto">
                {item.opintojenRahoitus && (
                  <span>
                    {'('}
                    <Koodistoviite value={item.opintojenRahoitus} />
                    {')'}
                  </span>
                )}
              </label>
...
const isLocalizedString = (obj) =>
  !R.isEmpty(R.intersection(['fi', 'en', 'sv'], Object.keys(obj)))

const formatLabel = (label) => capitalizeFirstChar(splitToWords(label))
const splitToWords = (str) =>
  str
    .split(/([A-ZÄÖÅ][a-zäöå]+)/g)
    .map(R.toLower)
    .join(' ')
    .trim()

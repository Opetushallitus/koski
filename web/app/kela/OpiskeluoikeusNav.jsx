    ...
    <div className="opiskeluoikeus-tabs">
      {selectedIndexAtom.map((selectedIndex) => (
        <>
          <ul>
            {opiskeluoikeudet.map((opiskeluoikeus, index) => {
              const oppilaitoksenNimi =
                opiskeluoikeus.oppilaitos &&
                t(opiskeluoikeus.oppilaitos.nimi || {})

              return (
                <li
                  onClick={() => selectedIndexAtom.set(index)}
                  className={index === selectedIndex ? 'selected' : ''}
                  key={index}
                >
                  <span className="opiskeluoikeuden-tyyppi">
                    {t(opiskeluoikeus.tyyppi.nimi || {})}
                  </span>
                  <span>
                    {oppilaitoksenNimi}{' '}
                    {opiskeluoikeudenTilaString(opiskeluoikeus)}
                  </span>
                  <ul>
          ...
          <KelaOpiskeluoikeus
            opiskeluoikeus={opiskeluoikeudet[selectedIndex]}
            henkilo={henkilo}
          />
        </>
      ))}

...
export const ExpandablePropertiesEditor = ({
  model,
  propertyName,
  propertyFilter = () => true
}) => {
  ...
  const wrappedModel = model.context.edit
    ? wrapOptional(propertyModel)
    : propertyModel
  const zeroVisibleProperties =
    modelProperties(wrappedModel, shouldShowProperty(model.context)).filter(
      propertyFilter
    ).length === 0

...
import {
  ...
  routeErrorP,
  titleKeyP
} from './virkailija/virkailijaRouter'
...
const topBarP = Bacon.combineWith(
  userP,
  titleKeyP,
  locationP,
  (user, titleKey, location) => (
    <TopBar
      user={user}
      titleKey={titleKey}
      inRaamit={inRaamit}
...
function Virkailija({ error, topBar, user, content, location }) {
  return (
    <div>
      <Error error={error} />
      {topBar}
...
titleKeyP.onValue((titleKey) => {
  ...
  document.querySelector('title').innerHTML = titleKey

...
test('Declining authorization surfaces an access_denied error', async ({ page }) => {
  await page.goto(gotoSample('/api/openid-api-test'))

  await loginKorhopankki(page, '280618-402H')

  await page.waitForURL('**/koski/omadata-oauth2/authorize**')
  await expect(page.getByRole('button', { name: 'Peruuta ja palaa' })).toBeVisible()
...
test('Invalid redirect_uri shows client error page', async ({ page }) => {
  ...
  await expect(page.getByLabel('Tapahtui virhe:')).toContainText('omadataoauth2-error-')
  await expect(page.getByLabel('Tapahtui virhe:')).toContainText('invalid_client_data')
})

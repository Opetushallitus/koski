  ...
  "Sure-rajapinnat" - {
    "/api/sure/oids" - {
      ...
      "Tuottaa oikean audit log viestin" in {
        ...
        postOids(oids) {
          verifyResponseStatusOk()
          AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "OPISKELUOIKEUS_HAKU", "target" -> Map("hakuEhto" -> "oids=1.2.246.562.24.90000000001,1.2.246.562.24.90000000002,...(4)")))
        }
      }

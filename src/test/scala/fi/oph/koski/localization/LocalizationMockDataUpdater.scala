...
object LocalizationMockDataUpdater extends App {
  val filename = "src/main/resources" + LocalizationConfig("koski").mockLocalizationResourceFilename
  val root = sys.env.getOrElse("VIRKAILIJA_ROOT", throw new RuntimeException("Environment variable VIRKAILIJA_ROOT missing"))

  val JArray(localizations) = new ReadOnlyRemoteLocalizationRepository(root, LocalizationConfig("koski"))(GlobalCacheManager).fetchLocalizations.asInstanceOf[JArray]

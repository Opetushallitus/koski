...
class AikuistenPerusopetuksenKurssikertymäRaporttiSpec extends FreeSpec with Matchers with RaportointikantaTestMethods with BeforeAndAfterAll with PutOpiskeluoikeusTestMethods[AikuistenPerusopetuksenOpiskeluoikeus] {
  ...
  private lazy val raportti =
    raporttiBuilder.build(List(jyväskylänNormaalikoulu), sqlDate("2006-01-01"), sqlDate("2016-12-30"))(session(defaultUser)).rows.map(_.asInstanceOf[AikuistenPerusopetuksenKurssikertymäRaporttiRow])
  ...
  "Aikuisten perusopetuksen oppijamäärien raportti" - {
    ...
    "Raportin kolumnit" in {
      ...
      r.yhteensäSuorituksia should equal(50)
      r.yhteensäTunnistettujaSuorituksia should equal(8)
      r.yhteensäTunnistettujaSuorituksiaRahoituksenPiirissä should equal(4)
      ...
      r.alkuvaiheenSuorituksia should equal(48)
      r.alkuvaiheenTunnistettujaSuorituksia should equal(4)
      r.alkuvaiheenTunnistettujaSuorituksiaRahoituksenPiirissä should equal(2)
    }
  }

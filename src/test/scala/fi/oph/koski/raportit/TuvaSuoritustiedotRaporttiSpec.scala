  ...
  "Tutkintokoulutukseen valmentavan koulutuksen suoritustietoraportti" - {
    ...
    "Raportti voidaan ladata ja lataaminen tuottaa auditlogin" in {
      authGet(s"api/raportit/tuvasuoritustietojentarkistus?oppilaitosOid=$stadinAmmattiopisto&alku=2018-01-01&loppu=2022-01-01&lang=fi&password=salasana") {
        ...
        AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "OPISKELUOIKEUS_RAPORTTI", "target" -> Map("hakuEhto" -> s"raportti=tuvasuoritustietojentarkistus&oppilaitosOid=$stadinAmmattiopisto&alku=2018-01-01&loppu=2022-01-01&lang=fi")))
      }
    }
    ...
    "Raportti voidaan ladata eri lokalisaatiolla ja lataaminen tuottaa auditlogin" in {
      authGet(s"api/raportit/tuvasuoritustietojentarkistus?oppilaitosOid=$stadinAmmattiopisto&alku=2018-01-01&loppu=2022-01-01&lang=sv&password=salasana") {
        ...
        response.bodyBytes.take(ENCRYPTED_XLSX_PREFIX.length) should equal(ENCRYPTED_XLSX_PREFIX)
        AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "OPISKELUOIKEUS_RAPORTTI", "target" -> Map("hakuEhto" -> s"raportti=tuvasuoritustietojentarkistus&oppilaitosOid=$stadinAmmattiopisto&alku=2018-01-01&loppu=2022-01-01&lang=sv")))
      }
    }
    ...
    "Raportti näyttää oikealta" - {
      ...
      "Sarakkeidein järjestys ammatillisen järjestysluvan taulukossa" in {
        sheets.find(_.title == "Ammatillisen järjestämislupa").get.columnSettings.map(_.title) should equal(Seq(
          ...
          "Opiskelijavuosikertymä (pv) - vanha laskentakaava (ennen v. 2026)",
          "Läsnä tai valmistunut (pv)",
          "Opiskelijavuosikertymä (pv) - nykyinen laskentakaava (v. 2026 eteenpäin)",
          "Opiskelijavuoteen kuuluvat lomat (pv)",
          "Muut lomat (pv)",
          "Majoitus (pv)",
          "Sisäoppilaitosmainen majoitus (pv)",
          "Vaativan erityisen tuen yhteydessä järjestettävä majoitus (pv)",
          "Erityinen tuki (pv)",
          "Vaativa erityisen tuen tehtävä (pv)",
          "Ulkomaanjaksot (pv)",
        ...
        verifyOppijanRows(KoskiSpecificMockOppijat.tuva,
          Seq(
            Map(
              ...
              "Päätason suorituksen nimi" -> Some("Tutkintokoulutukseen valmentava koulutus"),
              ...
              "Opiskelijavuosikertymä (pv) - vanha laskentakaava (ennen v. 2026)" -> 366.0,
              "Läsnä tai valmistunut (pv)" -> 366,
              "Opiskelijavuosikertymä (pv) - nykyinen laskentakaava (v. 2026 eteenpäin)" -> 0.0,
              "Opiskelijavuoteen kuuluvat lomat (pv)" -> 0,
              "Muut lomat (pv)" -> 0,
              "Majoitus (pv)" -> Some(0),
              ...
              "Vaativa erityisen tuen tehtävä (pv)" -> Some(0),
        ...
        verifyOppijanRows(KoskiSpecificMockOppijat.tuvaPerus,
          Seq(
            Map(
              ...
              "Oppijan master-oid" -> Some(KoskiSpecificMockOppijat.tuvaPerus.oid),

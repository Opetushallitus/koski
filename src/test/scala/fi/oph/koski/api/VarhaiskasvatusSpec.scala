...
class VarhaiskasvatusSpec extends FreeSpec with EsiopetusSpecification {
  "Varhaiskasvatuksen järjestäjä koulutustoimija" - {
    "voi luoda, lukea, päivittää ja mitätöidä päiväkodissa järjestettävän esiopetuksen opiskeluoikeuden organisaatiohierarkiansa ulkopuolelta" in {
      val resp = putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(koulutustoimija = hki), headers = authHeaders(MockUsers.helsinkiTallentaja) ++ jsonContent) {
      ...
      putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(oid = Some(oo.oid), koulutustoimija = hki, lisätiedot = None), headers = authHeaders(MockUsers.helsinkiTallentaja) ++ jsonContent) {
        verifyResponseStatusOk()
    ...
    "ei voi luoda perusopetuksessa järjestettävien esiopetuksen opiskeluoikeuksia organisaatiohierarkiansa ulkopuolelle" in {
      putOpiskeluoikeus(peruskouluEsiopetus(päiväkotiTouhula).copy(koulutustoimija = hki), headers = authHeaders(MockUsers.helsinkiTallentaja) ++ jsonContent) {
    ...
    "ei voi lukea, päivittää tai poistaa muiden luomia opiskeluoikeuksia organisaatiohierarkiansa ulkopuolelta" in {
      val esiopetuksenOpiskeluoikeus: EsiopetuksenOpiskeluoikeus = päiväkotiEsiopetus(päiväkotiTouhula).copy(koulutustoimija = hki)
      ...
      putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(oid = Some(oo.oid), koulutustoimija = hki, lisätiedot = None), headers = authHeaders(MockUsers.tornioTallentaja) ++ jsonContent) {
  ...
  "Koulutustoimija joka ei ole varhaiskasvatuksen järjestäjä" - {
    "ei voi luoda päiväkodissa järjestettävän esiopetuksen opiskeluoikeuden organisaatiohierarkiansa ulkopuolella" in {
      putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(koulutustoimija = jyväskylä), headers = authHeaders(MockUsers.jyväskyläTallentaja) ++ jsonContent) {
        verifyResponseStatus(403, KoskiErrorCategory.forbidden.organisaatio(s"Ei oikeuksia organisatioon ${MockOrganisaatiot.päiväkotiTouhula}"))
      }
    }
  }
  ...
  "Päiväkodin virkailija" - {
    "näkee kaikki omaan organisaatioon luodut opiskeluoikeudet" in {
      val resp1 = putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(koulutustoimija = hki), headers = authHeaders(MockUsers.helsinkiTallentaja) ++ jsonContent) {
        verifyResponseStatusOk()
        readPutOppijaResponse
      }
      val resp2 = putOpiskeluoikeus(päiväkotiEsiopetus(päiväkotiTouhula).copy(koulutustoimija = tornio), henkilö = asUusiOppija(ysiluokkalainen), headers = authHeaders(MockUsers.tornioTallentaja) ++ jsonContent) {
        verifyResponseStatusOk()
        readPutOppijaResponse
      }

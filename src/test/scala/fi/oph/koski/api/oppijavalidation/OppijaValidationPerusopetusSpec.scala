...
class OppijaValidationPerusopetusSpec extends TutkinnonPerusteetTest[PerusopetuksenOpiskeluoikeus] with KoskiHttpSpec with OpiskeluoikeusTestMethodsPerusopetus {
  ...
  "Laajuus" - {
    ...
    "Tukijaksot" - {
      ...
      val erityisenTuenPäätöstenViimeinenKäyttöpäivä = LocalDate.parse(KoskiApplicationForTests.config.getString("validaatiot.pidennetynOppivelvollisuudenViimeinenKäyttöpäivä"))
      ...
      "Erityisen tuen päätökset eivät saa alkaa viimeisen käyttöpäivän jälkeen" in {
        ...
        val oo = TuenPäätöksellinenBuilder(
          ooAlku = ooAlku,
          erityisenTuenPäätökset = erityisenTuenPäätökset(
            erityisenTuenPäätöstenViimeinenKäyttöpäivä.minusDays(31),
        ...
        setupOppijaWithOpiskeluoikeus(oo2) {
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date("Erityisen tuen päätöksen (1.9.2026-) viimeinen mahdollinen alkamispäivä on 31.8.2026."),
            KoskiErrorCategory.badRequest.validation.date("Erityisen tuen päätöksen (1.9.2026-) viimeinen mahdollinen päättymispäivä on 31.8.2026."),
          )
        }
      }
      ...
      "Tavoitekokonaisuuksittain opiskelu" - {
        ...
        "Luokka-aste tiedon siirtäminen vaatii tavoitekokonaisuuksittain opiskelun aikajakson jos vahvistettu tai arvioitu" in {
          ...
          setupOppijaWithOpiskeluoikeus(withLuokkaAste.copy(
            ...
            verifyResponseStatus(400,
              KoskiErrorCategory.badRequest.validation.date(
                "Perusopetuksen oppiaineen HI suorituksen tavoitekokonaisuuksittain opiskeluun liittyvä kenttä luokkaAste ei saa olla sama kuin vuosiluokka (8)"
              )
            )
          }
  ...
  "Opiskeluoikeuksien duplikaatit" - {
    ...
    "oppimäärän opiskeluoikeudet" - {
      ...
      "Samaa opiskeluoikeutta ei voi siirtää kahteen kertaan, vaikka opiskeluoikeuksilla on eri vuosiluokkien suoritukset" - {
        ...
        "samalla lähdejärjestelmän id:llä olemassaolevan opiskeluoikeuden päivittäminen onnistuu" in {
          setupOppijaWithOpiskeluoikeus(opiskeluoikeus.copy(lähdejärjestelmänId = lähdejärjestelmänId1), defaultHenkilö, headers = authHeaders(jyväskylänNormaalikoulunPalvelukäyttäjä) ++ jsonContent) {
            verifyResponseStatusOk()
          }
          putOppija(makeOppija(defaultHenkilö, List(opiskeluoikeus2.copy(lähdejärjestelmänId = lähdejärjestelmänId1))), headers = authHeaders(jyväskylänNormaalikoulunPalvelukäyttäjä) ++ jsonContent) {
            verifyResponseStatusOk()
          }
        }
      }
  ...
  "Opiskeluoikeudet oppivelvollisuuden pidennyksen ja tuen päätöksien muutosten siirtymäajan (1.8.2025 - 31.8.2026) yli" - {
    def makeOpiskeluoikeus(
    ...
    "Pidennetty oppivelvollisuus ja opetus oppiaineittain eivät saa olla päällekkäin" in {
      setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
        ...
        verifyResponseStatus(400,
          KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
            "Pidennetty oppivelvollisuus ja opetus oppiaineittain (vamman, sairauden tai rajoitteen perusteella) eivät saa olla päivämääriltään päällekkäin"
          ),
        )
      }
    }

    "Pidennetty oppivelvollisuus ja opetus toiminta-alueittain eivät saa olla päällekkäin" in {
      setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
        pidennettyOppivelvollisuus = Some(Aikajakso(date(2025, 8, 13), Some(date(2026, 8, 1)))),
        vammainen = Some(List(Aikajakso(date(2025, 8, 13), Some(date(2026, 8, 1))))),
        erityisenTuenPäätökset = Some(List(ErityisenTuenPäätös(Some(date(2025, 8, 13)), Some(date(2026, 7, 31))))),
        toimintaAlueittainOpiskelu = Some(List(Aikajakso(date(2026, 8, 1), Some(date(2026, 8, 3))))),
        tuenPäätöksenJaksot = Some(List(Tukijakso(Some(date(2026, 8, 1)), Some(date(2026, 8, 3))))),
      )) {
        verifyResponseStatus(400, KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
          "Pidennetty oppivelvollisuus ja opetus toiminta-alueittain (vamman, sairauden tai rajoitteen perusteella) eivät saa olla päivämääriltään päällekkäin"
        ))
      }
    }

    "Pidennettyä oppivelvollisuutta ei voi merkitä päättyväksi 31.8.2026 jälkeen" - {
    ...
    "Erityisen tuen jakso on sallittu välillä 1.8.2025-31.8.2026" in {
    ...
    "Pidennettyä oppivelvollisuutta ei voi merkitä sallittujen päivien ulkopuolelle opiskeluoikeudelle, joka alkaa rajapäivän (31.8.2026) jälkeen" - {
      "Suljettu aikajakso" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
          alkamisvuosi = 2027,
          pidennettyOppivelvollisuus = Some(Aikajakso(
            alku = Some(date(2027, 8, 1)),
            loppu = Some(date(2028, 6, 30)),
          )),
          vammainen = Some(List(Aikajakso(
            alku = Some(date(2027, 8, 1)),
            loppu = Some(date(2028, 6, 30)),
          ...
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus("Pidennetyn oppivelvollisuuden (1.8.2027-30.6.2028) viimeinen mahdollinen alkamispäivä on 31.8.2026."),
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus("Pidennetyn oppivelvollisuuden (1.8.2027-30.6.2028) viimeinen mahdollinen päättymispäivä on 31.8.2026."),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso("Vammaisuuden jakson (1.8.2027-30.6.2028) viimeinen mahdollinen alkamispäivä on 31.8.2026."),
      ...
      "Avoin aikajakso" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
          ...
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus("Pidennetyn oppivelvollisuuden (1.8.2027-) viimeinen mahdollinen alkamispäivä on 31.8.2026."),
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus("Pidennetyn oppivelvollisuuden (1.8.2027-) viimeinen mahdollinen päättymispäivä on 31.8.2026."),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso("Vaikeasti vammaisuuden jakson (1.8.2027-) viimeinen mahdollinen alkamispäivä on 31.8.2026."),
    ...
    "Opiskeluoikeus joka alkaa rajapäivän jälkeen on sallittu ilman pidennettyä oppivelvollisuutta" in {
      setupOppijaWithOpiskeluoikeus(PerusopetuksenOpiskeluoikeus(
        oppilaitos = Some(jyväskylänNormaalikoulu),
        koulutustoimija = None,
        suoritukset = List(perusopetuksenOppimääränSuoritusKesken),
        tila = NuortenPerusopetuksenOpiskeluoikeudenTila(List(
          NuortenPerusopetuksenOpiskeluoikeusjakso(date(2026, 9, 1), opiskeluoikeusLäsnä),
        )),
      )) {
        verifyResponseStatusOk()
      }
    ...
    "Pidennettyä oppivelvollisuutta ei voi merkitä päättyväksi 31.8.2026 jälkeen, vaikka opiskeluoikeus päättyy ennen rajapäivää" - {
      def makeOpiskeluoikeusEronnut(
        pidennettyOppivelvollisuus: Option[Aikajakso] = None,
        vammainen: Option[List[Aikajakso]] = None,
        vaikeastiVammainen: Option[List[Aikajakso]] = None,
        erityisenTuenPäätökset: Option[List[ErityisenTuenPäätös]] = None,
        ...
        tila = NuortenPerusopetuksenOpiskeluoikeudenTila(List(
          ...
          NuortenPerusopetuksenOpiskeluoikeusjakso(date(2026, 6, 30), opiskeluoikeusEronnut),
        )),
        lisätiedot = Some(PerusopetuksenOpiskeluoikeudenLisätiedot(
          pidennettyOppivelvollisuus = pidennettyOppivelvollisuus,
          vammainen = vammainen,
      ...
      "Suljettu aikajakso on kielletty, kun jaksot eivät ole sallituissa rajoissa" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeusEronnut(
          pidennettyOppivelvollisuus = Some(Aikajakso(
            alku = Some(date(2025, 8, 13)),
            loppu = Some(date(2026, 9, 1)),
          )),
          vammainen = Some(List(Aikajakso(
            alku = Some(date(2025, 8, 13)),
            loppu = Some(date(2026, 9, 1)),
          ...
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
              "Pidennetyn oppivelvollisuuden (13.8.2025-1.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso(
              "Vammaisuuden jakson (13.8.2025-1.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date(
              "Erityisen tuen päätöksen (13.8.2025-1.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
          )
        }
      }

      "Opiskeluoikeus joka päättyy ennen rajapäivää on sallittu, jos jaksot myös päättyvät ennen rajapäivää" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeusEronnut(
          pidennettyOppivelvollisuus = Some(Aikajakso(
            alku = Some(date(2025, 8, 13)),
            loppu = Some(date(2026, 6, 30)),
          )),
          vammainen = Some(List(Aikajakso(
            alku = Some(date(2025, 8, 13)),
            loppu = Some(date(2026, 6, 30)),
      ...
      "Avoin jakso sallittu, kun opiskeluoikeus on jo päättynyt ennen rajapäivää" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeusEronnut(
          pidennettyOppivelvollisuus = Some(Aikajakso(
            alku = Some(date(2025, 8, 13)),
            loppu = None,
    ...
    "Pidennettyä oppivelvollisuutta ei voi merkitä alkamaan 1.9.2026 tai sen jälkeen" - {
      "Suljettu aikajakso" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
          pidennettyOppivelvollisuus = Some(Aikajakso(
            alku = Some(date(2026, 9, 1)),
            loppu = Some(date(2026, 9, 30)),
          )),
          vammainen = Some(List(Aikajakso(
            alku = Some(date(2026, 9, 1)),
            loppu = Some(date(2026, 9, 30)),
          ...
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
              "Pidennetyn oppivelvollisuuden (1.9.2026-30.9.2026) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
              "Pidennetyn oppivelvollisuuden (1.9.2026-30.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso(
              "Vammaisuuden jakson (1.9.2026-30.9.2026) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso(
              "Vammaisuuden jakson (1.9.2026-30.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date(
              "Erityisen tuen päätöksen (1.9.2026-30.9.2026) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date(
              "Erityisen tuen päätöksen (1.9.2026-30.9.2026) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
          )
        }
      }
      ...
      "Avoin aikajakso" in {
        setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
          ...
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
              "Pidennetyn oppivelvollisuuden (1.9.2026-) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.pidennettyOppivelvollisuus(
              "Pidennetyn oppivelvollisuuden (1.9.2026-) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso(
              "Vaikeasti vammaisuuden jakson (1.9.2026-) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date.vammaisuusjakso(
              "Vaikeasti vammaisuuden jakson (1.9.2026-) viimeinen mahdollinen päättymispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date(
              "Erityisen tuen päätöksen (1.9.2026-) viimeinen mahdollinen alkamispäivä on 31.8.2026."
            ),
            KoskiErrorCategory.badRequest.validation.date(
    ...
    "Pidennetty oppivelvollisuus sallittu, kun jakso alkaa viimeistään 31.8.2026" in {
      setupOppijaWithOpiskeluoikeus(makeOpiskeluoikeus(
        pidennettyOppivelvollisuus = Some(Aikajakso(

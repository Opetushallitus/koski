...
class OppijaValidationLukio2019Spec extends AnyFreeSpec with PutOpiskeluoikeusTestMethods[LukionOpiskeluoikeus] with KoskiHttpSpec with OpiskeluoikeusTestMethodsLukio {
  "Laajuudet" - {
    ...
    "vahvistuksessa" - {
      "Yli 150 op ja yli 20 op valinnaisia sisältävän nuorten oppimäärän suorituksen pystyy vahvistamaan" in {
        setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(oppiainesuorituksetRiittääValmistumiseenNuorilla))))) {
          verifyResponseStatusOk()
        }
      }

      "Alle 150 op sisältävän nuorten oppimäärän suorituksen pystyy siirtämään vahvistamatta" in {
        setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(tila = LukionOpiskeluoikeudenTila(List(
      ...
      "Alle 150 op sisältävää nuorten oppimäärän suoritusta ei pysty vahvistamaan" in {
        val oo = poistaOmanÄidinkielenOpintojenSuoritukset(
          defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(oppiainesuorituksetEiRiitäValmistumiseen))))
        )
      ...
      "Alle 150 op sisältävää nuorten oppimäärän suoritusta pystyy vahvistamaan, jos siinä on oman äidinkielen opintojen suorituksia" in {
        val oo = defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(oppiainesuorituksetEiRiitäValmistumiseen))))
        setupOppijaWithOpiskeluoikeus(oo) {
          verifyResponseStatusOk()
        }
      }
      ...
      "Yli 150 op sisältävää nuorten oppimäärän suoritusta, jossa ei ole vähintään 20 op valinnaisia suorituksia, ei pysty vahvistamaan" in {
        ...
        val oo = poistaOmanÄidinkielenOpintojenSuoritukset(
          defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(osasuoritukset))))
        )
        setupOppijaWithOpiskeluoikeus(oo) {
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.tila.valmiiksiMerkityltäPuuttuuOsasuorituksia(
      ...
      "Yli 88 op sisältävän aikuisten oppimäärän suorituksen pystyy vahvistamaan" in {
        setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(
  ...
  "Diaarinumerot" - {
    "Nuorten ops" - {
      "Vain peruste OPH-2263-2019 sallitaan" in {
        ...
        setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(koulutusmoduuli = oppimääräAikuistenPerusteella)))) {
    ...
    "Aikuisten ops" - {
      "Vain peruste OPH-2267-2019 sallitaan" in {
        setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(oppimäärä = LukioExampleData.aikuistenOpetussuunnitelma)))) {
  ...
  "Vahvistus ja valmistuminen lukion oppimäärien suorituksessa" - {
    ...
    "Opiskeluoikeuden voi merkitä valmistuneeksi, jos siinä on arvioituja ja arvioimattomia osasuorituksia" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppiaineidenOppimäärienSuoritus.copy(osasuoritukset = Some(List(
  ...
  "Merkintä erityisestä tutkinnosta" - {
    "oppiainetasolla sallii oppiaineen ilman osasuorituksia ja suorituksen merkitsemisen vahvistetuksi vajaalla yhteislaajuudella" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(List(
        oppiaineenSuoritus(Lukio2019ExampleData.lukionÄidinkieli("AI1", pakollinen = true, laajuus = laajuus(5.0))).copy(suoritettuErityisenäTutkintona = true, arviointi = numeerinenLukionOppiaineenArviointi(9)).copy(osasuoritukset = None),
        oppiaineenSuoritus(Lukio2019ExampleData.matematiikka("MAB")).copy(arviointi = numeerinenLukionOppiaineenArviointi(9)).copy(osasuoritukset = Some(List(
    ...
    "oppiainetasolla sallii moduulin ja paikallisen opintojakson lisäämisen kyseiseen oppiaineeseen" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(osasuoritukset = Some(List(
        ...
        oppiaineenSuoritus(Lukio2019ExampleData.matematiikka("MAB")).copy(arviointi = numeerinenLukionOppiaineenArviointi(9), osasuoritukset = Some(List(
          moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("MAB2")).copy(arviointi = numeerinenArviointi(10)),
          moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("MAB3")).copy(arviointi = numeerinenArviointi(10)),
          moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("MAB4")).copy(arviointi = numeerinenArviointi(10))
        )))
    ...
    "suoritustasolla sallii osasuoritusten lisäämisen oppiaineisiin" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(aikuistenOppimääränSuoritus.copy(
        suoritettuErityisenäTutkintona = true,
        vahvistus = vahvistusPaikkakunnalla(päivä = date(2020, 5, 15)),
        osasuoritukset = Some(List(
    ...
    "suoritustasolla estetty jos kyseessä nuorten oppimäärän" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(
        suoritukset = List(oppimääränSuoritus.copy(
          suoritettuErityisenäTutkintona = true,
          osasuoritukset = Some(List(
            oppiaineenSuoritus(Lukio2019ExampleData.lukionÄidinkieli("AI1", pakollinen = true, laajuus = laajuus(5.0))).copy(arviointi = numeerinenLukionOppiaineenArviointi(9))
          )),
        ))
      )) {
        verifyResponseStatus(400, KoskiErrorCategory.badRequest.validation.rakenne.erityisenäTutkintonaSuoritettuNuortenOps())
      }
    }

    "oppiainetasolla ei salli ilman laajuutta" in {
      val oo = defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus.copy(
        osasuoritukset = Some(List(
          oppiaineenSuoritus(Lukio2019ExampleData.lukionÄidinkieli("AI1", pakollinen = true)).copy(
            suoritettuErityisenäTutkintona = true,
            ...
            osasuoritukset = Some(List(
              ...
              moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("ÄI2").copy(laajuus = laajuus(2.0))).copy(arviointi = numeerinenArviointi(8)),
        ...
        ) ::: oppiainesuorituksetRiittääValmistumiseenNuorilla.tail.tail)
      )))

      setupOppijaWithOpiskeluoikeus(oo) {
        verifyResponseStatus(400, KoskiErrorCategory.badRequest.validation.laajuudet.oppiaineenLaajuusPuuttuu(s"Erityisenä tutkintona suoritettavan oppiaineen koskioppiaineetyleissivistava/AI laajuus puuttuu"))
      }
    }
        ...
        osasuoritukset = Some(List(
          ...
          oppiaineenSuoritus(Lukio2019ExampleData.matematiikka("MAA", laajuus = laajuus(7.0))).copy(
            ...
            osasuoritukset = Some(List(
              ...
              moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("MAA3")).copy(arviointi = numeerinenArviointi(10)),
              moduulinSuoritusOppiaineissa(muuModuuliOppiaineissa("MAA4")).copy(arviointi = numeerinenArviointi(10))
            ))
          )
        ))
      )))
  ...
  "Suorituskieli" - {
    "Ei saa olla oppiainetasolla sama kuin päätason suorituksessa" in {
      setupOppijaWithOpiskeluoikeus(aktiivinenOpiskeluoikeus.copy(suoritukset = List(vahvistamatonOppimääränSuoritus.copy(suorituskieli = suomenKieli, osasuoritukset = Some(List(
  ...
  "Suoritukset" - {
    "Useampi lukionoppimaara-suoritus aiheuttaa virheen" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus, oppimääränSuoritus))) {
        verifyResponseStatus(400,
    ...
    "Ei voida olla samaan aikaan oppimäärän suorittaja ja aineopiskelija" in {
      setupOppijaWithOpiskeluoikeus(defaultOpiskeluoikeus.copy(suoritukset = List(oppimääränSuoritus,
  ...
  "oppimääräSuoritettu-kenttä" - {
    "Täytetään automaattisesti 'true'ksi kun oppimäärän suoritus vahvistettu" in {
      ...
      val opiskeluoikeus: LukionOpiskeluoikeus = setupOppijaWithAndGetOpiskeluoikeus(oo).asInstanceOf[LukionOpiskeluoikeus]
    ...
    "Ei täytetä automaattisesti, jos oppimäärän suoritusta ei ole vahvistettu" in {
      val oo = poistaOmanÄidinkielenOpintojenSuoritukset(defaultOpiskeluoikeus.copy(
        tila = LukionOpiskeluoikeudenTila(List(
          LukionOpiskeluoikeusjakso(alku = date(2019, 8, 1), tila = opiskeluoikeusAktiivinen, opintojenRahoitus = Some(ExampleData.valtionosuusRahoitteinen)),
        )),
        suoritukset = List(oppimääränSuoritus.copy(
    ...
    "Uskonto" - {
      ...
      "Biologian moduulia ei voi siirtää uskonnon suoritukseen" in {
        setupOppijaWithOpiskeluoikeus(opiskeluoikeus(List("BI1"), pakollinen = false)) {
          verifyResponseStatus(400,
            KoskiErrorCategory.badRequest.validation.rakenne(
              s"Moduulia BI1 ei voi siirtää oppiaineen/-määrän KT alle, koska se on oppiaineen/-määrän BI moduuli. Sallittuja KT-moduuleja ovat UE1, UE2, UE3, UE4, UE5, UE6, UO1, UO2, UO3, UO4, UO5, UO6, UK1, UK2, UK3, UK4, UK5, UK6, UI1, UI2, UI3, UI4, UI5, UI6, UJ1, UJ2, UJ3, UJ4, UJ5, UJ6, UBA1, UBA2, UBA3, UBA4, UBA5, UBA6, UBU1, UBU2, UBU3, UBU4, UBU5, UBU6, UKR1, UKR2, UKR3, UKR4, UKR5, UKR6, UX, UE1, UE2, UE3, UE4, UE5, UE6, UE1, UE2, UE3, UE4, UE5, UE6, UO1, UO2, UO3, UO4, UO5, UO6, UO1, UO2, UO3, UO4, UO5, UO6, UK1, UK2, UK3, UK4, UK5, UK6, UK1, UK2, UK3, UK4, UK5, UK6, UI1, UI2, UI3, UI4, UI5, UI6, UI1, UI2, UI3, UI4, UI5, UI6, UJ1, UJ2, UJ3, UJ4, UJ5, UJ6, UJ1, UJ2, UJ3, UJ4, UJ5, UJ6, UBA1, UBA2, UBA3, UBA4, UBA5, UBA6, UBA1, UBA2, UBA3, UBA4, UBA5, UBA6, UBU1, UBU2, UBU3, UBU4, UBU5, UBU6, UBU1, UBU2, UBU3, UBU4, UBU5, UBU6, UKR1, UKR2, UKR3, UKR4, UKR5, UKR6, UKR1, UKR2, UKR3, UKR4, UKR5, UKR6, UX, ET1, ET2, ET3, ET4, ET5, ET6."
            )
          )
        }
      }
  ...
  private def poistaOmanÄidinkielenOpintojenSuoritukset(oo: LukionOpiskeluoikeus) =
    oo.copy(suoritukset = oo.suoritukset.map {
      case s: LukionOppimääränSuoritus2019 => s.copy(omanÄidinkielenOpinnot = None)
      case s: Any => s
    })

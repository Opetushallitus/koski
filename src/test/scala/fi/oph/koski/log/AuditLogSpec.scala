...
import fi.oph.koski.koskiuser.{MockUsers, KoskiUser$}
  ...
  "AuditLog" - {
    "Logs in JSON format" in {
      verifyLogMessage(AuditLogMessage(
        KoskiOperation.OPISKELUOIKEUS_LISAYS, MockUsers.hiiri.asTorUser, Map(KoskiMessageField.oppijaHenkiloOid ->  "1.2.246.562.24.00000000001")),

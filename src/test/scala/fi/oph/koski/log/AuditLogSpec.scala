...
class AuditLogSpec extends FreeSpec with Assertions with Matchers {
  ...
  "AuditLog" - {
    "Logs in JSON format" in {
      audit.log(KoskiAuditLogMessage(KoskiOperation.OPISKELUOIKEUS_LISAYS, MockUsers.omniaPalvelukäyttäjä.toKoskiSpecificSession(käyttöoikeuspalvelu), Map(KoskiAuditLogMessageField.oppijaHenkiloOid ->  "1.2.246.562.24.00000000001")))

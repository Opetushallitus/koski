...
class VirtaXMLConverterSpec extends AnyFreeSpec with TestEnvironment with Matchers with OptionValues {
  ...
  "Suoritusten konvertointi" - {
    ...
    "Lähdeorganisaatio" - {
      def opintosuoritus(organisaatiot: Seq[Elem]): Elem =
        <virta:Opintosuoritus opiskeluoikeusAvain="avopH1O1" opiskelijaAvain="avopH1" koulutusmoduulitunniste="K-127" avain="lahde-1">
          <virta:SuoritusPvm>2017-12-04</virta:SuoritusPvm>
          <virta:Laajuus><virta:Opintopiste>5</virta:Opintopiste></virta:Laajuus>
          <virta:Myontaja>10076</virta:Myontaja>
          <virta:Laji>2</virta:Laji>
          <virta:Nimi kieli="fi">Kurssi</virta:Nimi>
          {organisaatiot}
          <virta:HyvaksilukuPvm>2018-01-15</virta:HyvaksilukuPvm>
        </virta:Opintosuoritus>

      def organisaatio(rooli: String, koodi: String): Elem =
        <virta:Organisaatio><virta:Rooli>{rooli}</virta:Rooli><virta:Koodi>{koodi}</virta:Koodi></virta:Organisaatio>

      def lähde(organisaatiot: Seq[Elem]): Option[KorkeakoulunLähdeorganisaatio] =
        convertSuoritus(opintosuoritus(organisaatiot))
          .value.asInstanceOf[KorkeakoulunOpintojaksonSuoritus].lähdeorganisaatio

      "oppilaitosnumero ratkaistaan oppilaitokseksi" in {
        val l = lähde(Seq(organisaatio("3", "10076"))).value
        l.koodi shouldBe "10076"
        l.oppilaitos.value.oppilaitosnumero.value.koodiarvo shouldBe "10076"
      }

      "ulkomaisen organisaation koodi säilyy ilman oppilaitosta" in {
        val l = lähde(Seq(organisaatio("3", "UK"))).value
        l.koodi shouldBe "UK"
        l.oppilaitos shouldBe None
      }

      "muissa rooleissa olevia organisaatioita ei lueta" in {
        lähde(Seq(organisaatio("2", "10076"), organisaatio("5", "01905"))) shouldBe None
      }

      "on None jos Organisaatio puuttuu" in {
        lähde(Nil) shouldBe None
      }
    }

    "Opintosuorituksen laji" - {
      def opintosuoritus(suoritusAvain: String, lajiArvo: Option[String], sisältyvät: Seq[String] = Nil): Elem =
        <virta:Opintosuoritus opiskeluoikeusAvain="avopH1O1" opiskelijaAvain="avopH1" koulutusmoduulitunniste="K-126" avain={suoritusAvain}>
          <virta:SuoritusPvm>2017-12-04</virta:SuoritusPvm>
          <virta:Laajuus><virta:Opintopiste>5</virta:Opintopiste></virta:Laajuus>
          <virta:Myontaja>10076</virta:Myontaja>
          {lajiArvo.map(l => <virta:Laji>{l}</virta:Laji>).getOrElse(scala.xml.NodeSeq.Empty)}
          <virta:Nimi kieli="fi">Kurssi</virta:Nimi>
          {sisältyvät.map(a => <virta:Sisaltyvyys sisaltyvaOpintosuoritusAvain={a}/>)}
        </virta:Opintosuoritus>

      def lajinKoodiarvo(suoritus: Elem): Option[String] =
        convertSuoritus(suoritus).value.asInstanceOf[KorkeakoulunOpintojaksonSuoritus].laji.map(_.koodiarvo)

      // Juuritasolla convertSuoritus valitsee suoritustyypin lajin perusteella, eikä muita kuin
      // lajit 1 ja 2 konvertoida lainkaan. Laji luetaan siis vain sisältyvältä suoritukselta.
      def sisältyvänLaji(lapsenLaji: Option[String]): Option[String] = {
        val lapsi = opintosuoritus("laji-lapsi", lapsenLaji)
        val vanhempi = opintosuoritus("laji-vanhempi", Some("2"), Seq("laji-lapsi"))
        converter.convertSuoritus(None, vanhempi, List(vanhempi, lapsi))
          .value.asInstanceOf[KorkeakoulunOpintojaksonSuoritus]
          .osasuoritukset.value.head.laji.map(_.koodiarvo)
      }

      "parsitaan koodistoviitteeksi" in {
        lajinKoodiarvo(opintosuoritus("laji-a", Some("2"))) shouldBe Some("2")
      }

      "on None jos Laji puuttuu" in {
        sisältyvänLaji(None) shouldBe None
      }

      "tuntematon arvo ei kaada konversiota" in {
        sisältyvänLaji(Some("9")) shouldBe None
      }

      "juuritason suoritusta ei konvertoida jos laji ei ole tutkinto tai opintosuoritus" in {
        convertSuoritus(opintosuoritus("laji-juuri-3", Some("3"))) shouldBe None
        convertSuoritus(opintosuoritus("laji-juuri-9", Some("9"))) shouldBe None
        convertSuoritus(opintosuoritus("laji-juuri-ei", None)) shouldBe None
      }

      "sisältyvä 'ei huomioitava' -suoritus säilyy osasuorituksena ja erottuu lajista" in {
        val lapsi = opintosuoritus("laji-lapsi", Some("3"))
        val vanhempi = opintosuoritus("laji-vanhempi", Some("2"), Seq("laji-lapsi"))
        val suoritus = converter.convertSuoritus(None, vanhempi, List(vanhempi, lapsi))
          .value.asInstanceOf[KorkeakoulunOpintojaksonSuoritus]
        suoritus.laji.map(_.koodiarvo) shouldBe Some("2")
        suoritus.osasuoritukset.value.map(_.laji.map(_.koodiarvo)) shouldBe List(Some("3"))
      }
    }

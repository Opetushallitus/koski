  ...
  "Virta-opiskeluoikeuksien konvertointi" - {
    ...
    "Patevyys" - {
      "parsitaan opintosuorituksilta sekä jaksoilta ilman duplikaatteja ja vain kaksimerkkiset kirjainkoodit huomioidaan" in {
        ...
        val opettajanPatevyydet = oo.head.lisätiedot.flatMap(_.opettajanPätevyys.map(x => x.map(_.koodiarvo))).getOrElse(List())
        val ainePatevyydet = oo.head.lisätiedot.flatMap(_.opetettavanAineenPätevyys.map(x => x.map(_.koodiarvo))).getOrElse(List())

...
trait RaportointikantaTestMethods extends KoskiHttpSpec {
  ...
  lazy val confidentalRaportointiDb: ConfidentialRaportointiDatabase = mainRaportointiDb.confidential.get
  ...
  lazy val tempConfidentalRaportointiDb: ConfidentialRaportointiDatabase = tempRaportointiDb.confidential.get
  ...
  def verifyRaportinLataaminen(apiUrl: String, expectedRaporttiNimi: String, expectedFileNamePrefix: String, lang: String = "fi"): Unit = {
    ...
    authGet(s"$apiUrl?$queryString1&$queryString2") {
      ...
      AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "OPISKELUOIKEUS_RAPORTTI", "target" -> Map("hakuEhto" -> s"raportti=$expectedRaporttiNimi&$queryString1")))
    }
  }

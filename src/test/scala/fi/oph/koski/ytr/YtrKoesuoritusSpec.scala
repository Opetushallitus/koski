...
class YtrKoesuoritusSpec extends AnyFreeSpec with KoskiHttpSpec with OpiskeluoikeusTestMethods {
  "Kansalainen" - {
    ...
    "näkee koesuorituksensa (HTML)" in {
      get("koesuoritus/1234S/html/1234S_YY_420.html", headers = kansalainenLoginHeaders("080698-703Y")) {
        verifyResponseStatusOk()
    ...
    "ei näe toisten koesuoritusta" in {
      get("koesuoritus/2345K/pdf/2345K_XX_12345.pdf", headers = kansalainenLoginHeaders("210244-374K")) {
        verifyResponseStatus(404, Nil)
      }
    }

    "ei näe toisen huollettavan koesuoritusta" in {
      get(s"koesuoritus/2345K/pdf/2345K_XX_12345.pdf?huollettava=${KoskiSpecificMockOppijat.ylioppilasLukiolainen.oid}", headers = kansalainenLoginHeaders(KoskiSpecificMockOppijat.amis.hetu.get)) {
        verifyResponseStatus(404, Nil)
      }
    }
    ...
    "oman koesuorituksen haku aiheuttaa auditlogin" in {
      ...
      AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "KANSALAINEN_YLIOPPILASKOE_HAKU"))
    }
    ...
    "huollettavan koesuorituksen haku aiheuttaa auditlogin" in {
      ...
      AuditLogTester.verifyLastAuditLogMessageForOperation(Map("operation" -> "KANSALAINEN_HUOLTAJA_YLIOPPILASKOE_HAKU"))
    }
  }

  "Viranomainen" - {
    "ei näe koesuoritusta" in {
      authGet("koesuoritus/2345K/pdf/2345K_XX_12345.pdf", defaultUser) {

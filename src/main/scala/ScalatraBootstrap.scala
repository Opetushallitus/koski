...
import fi.oph.koski.log.{LocalAuditLogServlet, Logging}
import fi.oph.koski.luovutuspalvelu.{LuovutuspalveluServlet, PalveluvaylaServlet}
...
class ScalatraBootstrap extends LifeCycle with Logging with GlobalExecutionContext {
  override def init(context: ServletContext) = try {
    ...
    mount("/api/auditlog", new LocalAuditLogServlet)

...
class TorUser(val oid: String, val organisationOidsObservable: Observable[Set[String]]) extends Loggable with Logging {
...
object TorUser {
  def apply(oid: String, userOrganisationsRepository: UserOrganisationsRepository) = {
    new TorUser(oid, userOrganisationsRepository.getUserOrganisations(oid))
  }
}
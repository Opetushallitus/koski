...
object MockUsers extends UserOrganisationsRepository with DirectoryClient {
  case class MockUser(ldapUser: LdapUser, organisaatiot: Set[String]) {
    def asTorUser = TorUser(ldapUser.oid, organisaatiot)
  ...
  def getUserOrganisations(oid: String): Set[String] = users.map(user => (user.oid, user.organisaatiot)).toMap.getOrElse(oid, Set.empty)

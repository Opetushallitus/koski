...
object UserRepository {
  def apply(config: Config): UserRepository = {
    if (config.hasPath("authentication-service")) {
      CachingProxy(config, TimedProxy[UserRepository](new RemoteUserRepository(AuthenticationServiceClient(config), TimedProxy[OrganisaatioRepository](new RemoteOrganisaatioRepository(config)))))
    } else {

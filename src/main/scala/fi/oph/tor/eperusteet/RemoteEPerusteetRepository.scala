...
class RemoteEPerusteetRepository(ePerusteetRoot: String) extends EPerusteetRepository {
  ...
  def findPerusteet(query: String) = {
    http("/api/perusteet?sivukoko=100&nimi=" + query)(Http.parseJson[EPerusteet]).data
  }

  def findPerusteetByDiaarinumero(diaarinumero: String) = {
    http("/api/perusteet?diaarinumero=" + diaarinumero)(Http.parseJson[EPerusteet]).data
  }

  def findRakenne(diaariNumero: String): Option[EPerusteRakenne] = {
    http(s"/api/perusteet/diaari?diaarinumero=$diaariNumero")(Http.parseJsonOptional[EPerusteTunniste])
      .map(e => http("/api/perusteet/" + e.id + "/kaikki")(Http.parseJson[EPerusteRakenne]))
  }
}

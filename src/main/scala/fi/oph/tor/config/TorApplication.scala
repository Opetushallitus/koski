...
class TorApplication(val config: Config) extends Logging {
  ...
  lazy val virta = TimedProxy[OpiskeluOikeusRepository](VirtaOpiskeluoikeusRepository(VirtaClient(config), oppijaRepository, oppilaitosRepository, koodistoViitePalvelu, validator))

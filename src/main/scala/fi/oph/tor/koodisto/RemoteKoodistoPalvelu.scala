...
class RemoteKoodistoPalvelu(virkailijaUrl: String) extends KoodistoPalvelu with Logging {
  ...
  def getKoodistoKoodit(koodisto: KoodistoViite): Option[List[KoodistoKoodi]] = {
    runTask(http("/koodisto-service/rest/codeelement/codes/" + koodisto + noCache) {
      case (404, _, _) => None
  ...
  def getKoodisto(koodisto: KoodistoViite): Option[Koodisto] = {
    runTask(http("/koodisto-service/rest/codes/" + koodisto + noCache)(Http.parseJsonOptional[Koodisto]))
  }

  def getLatestVersion(koodisto: String): Option[KoodistoViite] = {
    val latestKoodisto: Option[KoodistoWithLatestVersion] = runTask(http("/koodisto-service/rest/codes/" + koodisto + noCache)(Http.parseJsonIgnoreError[KoodistoWithLatestVersion]))

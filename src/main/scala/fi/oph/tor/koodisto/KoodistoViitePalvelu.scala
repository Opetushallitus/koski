...
case class KoodistoViitePalvelu(koodistoPalvelu: KoodistoPalvelu) extends Logging {
  def getKoodistoKoodiViitteet(koodisto: KoodistoViite): Option[List[Koodistokoodiviite]] = {
    koodistoPalvelu.getKoodistoKoodit(koodisto).map { _.map { koodi => Koodistokoodiviite(koodi.koodiArvo, Some(koodi.nimi), koodi.lyhytNimi("fi"), koodisto.koodistoUri, Some(koodisto.versio))} }
  }

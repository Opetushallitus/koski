...
trait OpiskeluOikeusRepository {
  ...
  def createOrUpdate(oppijaOid: PossiblyUnverifiedOppijaOid, opiskeluOikeus: OpiskeluOikeus)(implicit userContext: UserContext): Either[HttpStatus, CreateOrUpdateResult]
}

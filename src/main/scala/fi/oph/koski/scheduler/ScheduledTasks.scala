...
class ScheduledTasks(application: KoskiApplication) {
  val updateHenkilötScheduler =
    new Scheduler(application.database.db, "henkilötiedot-update", new FixedTimeOfDayScheduling(5, 40), () => updateHenkilöt())

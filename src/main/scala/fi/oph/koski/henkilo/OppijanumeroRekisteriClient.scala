...
case class OppijanumeroRekisteriClient(config: Config) {
  ...
  def findOppijatByHetus(hetus: List[String]): Task[List[OppijaHenkilö]] =
    oidServiceHttp.post(uri"/oppijanumerorekisteri-serivce/henkilo/henkiloPerustietosByHenkiloHetuList", hetus)(json4sEncoderOf[List[String]])(Http.parseJson[List[OppijaNumerorekisteriOppija]]).map(_.map(_.toOppijaHenkilö))
}

...
object SchemaExport {
  ...
  private def parseSchema(schema: Schema): TypeModel = schema match {
  ...
  private def parseClassSchema(classSchema: ClassSchema): ObjectType = {
    val propTypes = classSchema.properties.map(_.schema).map(parseSchema)
    ...
    val propsOptionalDueDefaultValue = classSchema.properties.filter { p =>
      !p.schema.isInstanceOf[OptionalSchema] && DefaultValue.getDefaultValue(p.metadata).isDefined
    }
    model.makeOptional(propsOptionalDueDefaultValue.map(_.key))
  }
}

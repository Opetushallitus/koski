...
case class TiedonsiirtoDocument(
  ...
  lähdejärjestelmä: Option[String],
  aikaleima: Timestamp,
  requestId: Option[String] = Some(Random.alphanumeric.take(6).mkString)
) {
  def id: String = tallentajaOrganisaatioOid + "_" + oppijaId

  private def oppijaId: String = oppija.flatMap(h => h.hetu.orElse(h.oid)).orElse(requestId).getOrElse("")
}

...
object KoskiTables {
  ...
  trait OpiskeluoikeusTableCompanion[OOROW <: OpiskeluoikeusRow] {
    protected val serializationContext: SerializationContext = SerializationContext(KoskiSchema.schemaFactory, KoskiSchema.skipComputedProperties)
      ...
      for {
        ...
        if (oo.oppilaitosOid inSet oppilaitosOidit) ||
           ...
           ((oo.koulutusmuoto === OpiskeluoikeudenTyyppi.esiopetus.koodiarvo) &&
             oo.koulutustoimijaOid.map(_ inSet varhaiskasvatusOstopalveluoikeudet.map(_.ostavaKoulutustoimija.oid)).getOrElse(false)) ||

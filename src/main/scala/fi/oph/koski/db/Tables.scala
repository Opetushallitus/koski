    ...
    val query = (if (user.hasGlobalReadAccess || user.hasGlobalKoulutusmuotoReadAccess) {
      ...
      for {
        ...
        if (oo.oppilaitosOid inSet oppilaitosOidit) ||
           ...
           (oo.oppilaitosOid inSet varhaiskasvatusOikeudet.map(_.organisaatio.oid)) && oo.koulutustoimijaOid.map(_ inSet varhaiskasvatusOikeudet.map(_.koulutustoimija.oid)).getOrElse(false)
      } yield oo

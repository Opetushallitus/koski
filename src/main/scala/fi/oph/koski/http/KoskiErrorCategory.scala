  ...
  object badRequest extends ErrorCategory("badRequest", 400, "Epäkelpo syöte") {
    ...
    class Validation extends ErrorCategory(badRequest, "validation", "Syötteen validointi epäonnistui") {
      ...
      class Tila extends ErrorCategory(Validation.this, "tila", "Suorituksen tilaan liittyvä validointivirhe") {
        ...
        val nuortenPerusopetuksenValmistunutTilaIlmanYsiluokanSuoritusta = subcategory("nuortenPerusopetuksenValmistunutTilaIlmanYsiluokanSuoritusta", "Kun nuorten perusopetuksen oppimäärä on suoritettu, tulee opiskeluoikeudella olla myös 9. luokka-asteen suoritus paitsi jos kyseessä on vuosiluokkiin sitomaton opetus; opiskelija on ollut kotiopetuksessa päättötodistuksen vahvistuspäivämääränä; tai päättötodistuksen suoritustapa on \"erityinen tutkinto\"")
      }
      val tila = new Tila

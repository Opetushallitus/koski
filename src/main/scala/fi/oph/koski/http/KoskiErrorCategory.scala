...
object KoskiErrorCategory {
  ...
  object badRequest extends ErrorCategory("badRequest", 400, "Epäkelpo syöte") {
    ...
    class Massaluovutus extends ErrorCategory(badRequest, "massaluovutus", "Virheellinen kysely") {
    ...
    val massaluovutus = new Massaluovutus
    ...
    class Validation extends ErrorCategory(badRequest, "validation", "Syötteen validointi epäonnistui") {
      ...
      class Organisaatio extends ErrorCategory(Validation.this, "organisaatio", "Epäkelpo organisaatio") {
        ...
        val hankintakoulutus = subcategory("hankintakoulutus", "Opiskeluoikeutta ei voi tallentaa oman organisaation toimipaikkaan jos kyseessä on hankintakoulutus")
      ...
      class Rakenne extends ErrorCategory(Validation.this, "rakenne", "Tutkinnon rakenteeseen liittyvä validointivirhe") {
        ...
        val tuntematonTutkintonimike = subcategory("tuntematonTutkintonimike", "Annettua tutkintonimikettä ei löydy tutkinnon rakenteesta.")
        ...
        val päällekkäisetETjaKTSuoritukset = subcategory("päällekkäisetETjaKTsuoritukset","Lukion oppimäärän suorituksessa ei voi olla sekä ET- että KT-oppiaineen suorituksia")
      ...
      class Tila extends ErrorCategory(Validation.this, "tila", "Suorituksen tilaan liittyvä validointivirhe") {
        ...
        val tilanRahoitusmuotoEiSaaOllaJotpa = subcategory("tilanRahoitusmuotoEiSaaOllaJotpa", "Tälle opiskeluoikeudelle ei ole mahdollista valita Jatkuvan oppimisen ja työllisyyden palvelukeskuksen rahoitusmuotoa")
        val tilanRahoitusmuodonYhtenäisyys = subcategory("tilanRahoitusmuodonYhtenäisyys", "Opiskeluoikeudella, joka on Jatkuvan oppimisen ja työllisyyden palvelukeskuksen rahoittama, ei rahoitusmuoto voi vaihtua opiskeluoikeuden aikana")
        ...
        val ahvenanmaanPerusopetuksenOppimääräIlmanYsiluokanSuoritusta = subcategory("ahvenanmaanPerusopetuksenOppimääräIlmanYsiluokanSuoritusta", "Ahvenanmaan perusopetuksen opiskeluoikeudesta, jossa on vahvistettu perusopetuksen oppimäärän suoritus, tulee löytyä myös vahvistettu 9. vuosiluokan suoritus paitsi niissä tapauksissa, joissa oppilas on KOSKI-tietovarantoon siirretyn datan perusteella kotiopetuksessa perusopetuksen oppimäärän vahvistuspäivänä tai joissa perusopetuksen oppimäärän suoritustapa on erityinen tutkinto.")
      ...
      class Laajuudet extends ErrorCategory(Validation.this, "laajuudet", "Suoritusten laajuuksiin liittyvä validointivirhe") {
        ...
        val lops2015VääräLaajuusAikuiset = subcategory("lops2015VääräLaajuusAikuiset", "Lukion aikuisten oppimäärän LOPS2015 voi merkitä valmiiksi vain kun vähintään 44 kurssia on suoritettu")
        val lops2015VääräLaajuusNuoret = subcategory("lops2015VääräLaajuusNuoret", "Lukion nuorten oppimäärän LOPS2015 voi merkitä valmiiksi vain kun vähintään 75 kurssia on suoritettu")
        ...
        val suorituksenLaajuusEiVastaaRakennetta = subcategory("suorituksenLaajuusEiVastaaRakennetta", "Suorituksen laajuus ei vastaa rakennetta")
        val osasuoritusVääräLaajuus = subcategory("osasuoritusVääräLaajuus", "Osasuorituksella on väärä laajuus")
      }
      ...
      class Ammatillinen extends ErrorCategory(Validation.this, "ammatillinen", "Ammatilliseen opiskeluoikeuteen liittyvä validointivirhe") {
        ...
        val lomaTilaRajapäivänJälkeen = subcategory("lomaTilaRajapäivänJälkeen", "Opiskeluoikeuden loma-tila ei voi olla voimassa 31.12.2025 jälkeen")
        def lisätietoRajapäivänJälkeenAlkavaOpiskeluoikeus(lisätiedonNimi: String) = subcategory("lisätietoRajapäivänJälkeen", s"$lisätiedonNimi lisätieto ei ole käytössä 31.12.2025 jälkeen alkavalle opiskeluoikeudelle")
        def lisätietoAlkaaRajapäivänJälkeen(lisätiedonNimi: String) = subcategory("lisätietoRajapäivänJälkeen", s"$lisätiedonNimi aikajakso ei voi alkaa 31.12.2025 jälkeen")
      ...
      class Perusopetus extends ErrorCategory(
        Validation.this,
        "perusopetus",
        "Perusopetuksen kirjauksiin liittyvä validointivirhe"
      ) {
        val valmistavanLisäopetusEiSallittuEnnen = subcategory("valmistavanLisäopetusEiSallittuEnnen", "Valmistavan lisäopetus -lisätietoa voi käyttää vain 1.8.2026 tai sen jälkeen alkavissa jaksoissa")
  ...
  object notFound extends ErrorCategory("notFound", 404, "Not found") {
    ...
    val oppijaaEiLöydyHetulla = subcategory("oppijaaEiLöydyHetulla", "Oppijaa ei löydy annetulla hetulla.")
    val huoltajaaEiLöydyHetulla = subcategory("huoltajaaEiLöydy", "VTJ huoltajaa ei löydy annetulla hetulla.")
    val hetuaEiLöydy = subcategory("hetuaEiLöydy", "Oppijalla ei ole hetua - VTJ huollettavia ei voida hakea.")
  ...
  object unavailable extends ErrorCategory("unavailable", 503, "Service unavailable") {
    ...
    val ovara = subcategory("ovara", "Valintatietoja ei juuri nyt saada haettua. Yritä myöhemmin uudelleen.")

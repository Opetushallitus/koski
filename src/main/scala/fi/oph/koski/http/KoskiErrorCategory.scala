  ...
  object badRequest extends ErrorCategory("badRequest", 400, "Epäkelpo syöte") {
    ...
    class Validation extends ErrorCategory(badRequest, "validation", "Syötteen validointi epäonnistui") {
      ...
      class Date extends ErrorCategory(Validation.this, "date", "Päivämäärä on oikeassa formaatissa, mutta semanttisesti epäkelpo.") {
        ...
        val alkamispäivä = subcategory("alkupäivämäärä", "Opiskeluoikeuden alkamispäivä ei vastaa ensimmäisen opiskeluoikeusjakson alkupäivää")
        val päättymispäivämäärä = subcategory("loppupäivämäärä", "Opiskeluoikeuden päättymispäivä ei vastaa viimeisen opiskelut päättävän opiskeluoikeusjakson alkupäivää")
      }
      val date = new Date

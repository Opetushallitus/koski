...
class KoskiWebApplication(application: KoskiApplication) extends Logging with GlobalExecutionContext {
  private var mounts: List[(String, Handler)] = List.empty

  def mount(path: String, handler: Handler) = mounts ++ List((path, handler))
  ...
  if (Fixtures.shouldUseFixtures(application.config)) {
    mount("/fixtures", new FixtureServlet(application))
  ...
  def mountTo(context: RichServletContext) = mounts.toList.foreach { case (path, handler) => context.mount(handler, path) }
}
...
import fi.oph.koski.ovara.{OpiskelijavalintatietoConverter, OvaraClient}
...
class KoskiApplication(
  ...
  lazy val kiosService = new KiosService(this)
  ...
  lazy val opiskelijavalintatietoConverter = new OpiskelijavalintatietoConverter(koodistoViitePalvelu)

    ...
    try {
      for {
        response <- runIO(
          ...
          .toRight(KoskiErrorCategory.unavailable.todistus.stampingError(s"invalid response from AIS for todistus ${req.SignRequest.`@RequestID`}"))

        result <- if (response.isSuccess) { Right(response) } else { Left(response.SignResponse.Result.toHttpStatus) }
      } yield result
    } catch {
      case e: Exception =>
        val error = s"Error requesting signature for todistus ${req.SignRequest.`@RequestID`}"
        logger.error(e)(error)
        Left(KoskiErrorCategory.unavailable.todistus.stampingError(error))
    }
  }
}

...
class KoskiDatabaseFixtureCreator(database: KoskiDatabase, repository: OpiskeluOikeusRepository, oppijaRepository: OppijaRepository, validator: KoskiValidator) extends KoskiDatabaseMethods with Timing {
  ...
  private def defaultOpiskeluOikeudet = {
    List((MockOppijat.eero.oid, OpiskeluOikeusTestData.opiskeluOikeus(MockOrganisaatiot.stadinAmmattiopisto)),
      ...
      (MockOppijat.koululainen.oid, ExamplesEsiopetus.esioppilas.opiskeluoikeudet.head),
      ...
      (MockOppijat.ammattilainen.oid, AmmatillinenPerustutkintoExample.perustutkinto.opiskeluoikeudet.head),
      ...
      (MockOppijat.omattiedot.oid, ExamplesPerusopetus.päättötodistus.opiskeluoikeudet.head),
      (MockOppijat.omattiedot.oid, ExamplesLukio.päättötodistus),
      (MockOppijat.ibOpiskelija.oid, ExamplesIB.opiskeluoikeus)
    )
  }
}

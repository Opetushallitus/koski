    ...
    if (!fixtureCacheCreated) {
      cachedPerustiedot = Some(validatedOpiskeluoikeudet.map { case (henkilö, oo) =>
        val opiskeluoikeus = if (henkilö.hetu.contains(MockOppijat.organisaatioHistoria.hetu.get)) {
          oo.asInstanceOf[AmmatillinenOpiskeluoikeus].copy(opiskeluoikeudenOrganisaatiohistoria = Some(AmmatillinenExampleData.opiskeluoikeudenOrganisaatioHistoria)) // organisaatioHistoria is cleared from incoming data, so we have to sneak it in
        } else {
          oo
        }

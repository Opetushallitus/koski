...
import fi.oph.koski.log.{AuditLog, KoskiAuditLogMessage, AuditLogMessageField}
...
class YtrKoesuoritusServlet(implicit val application: KoskiApplication) extends OppijaHtmlServlet with RequiresKansalainen {
  ...
  private def mkAuditLog(oid: String, operation: KoskiOperation): Unit = AuditLog.log(KoskiAuditLogMessage(operation, session, Map(AuditLogMessageField.oppijaHenkiloOid -> oid)))

...
case class ListModel(items: List[EditorModel]) extends EditorModel {
...
case class OptionalModel(model: Option[EditorModel]) extends EditorModel {
  override def empty = !model.exists(!_.empty)
}

case class OneOfModel(`class`: String, model: EditorModel) extends EditorModel {
...
object EditorModelSerializer extends Serializer[EditorModel] {
  ...
  override def serialize(implicit format: Formats): PartialFunction[Any, JValue] = {
    case (m: EditorModel) => {
      val json = m match {
        ...
        case (OptionalModel(model)) => d("optional", "model" -> model)
        case (ListModel(items)) => d("array", "items" -> items)
        case (EnumeratedModel(value, alternatives)) => d("enum", "value" -> value, "alternatives" -> alternatives)
        case (OneOfModel(c, model)) => d("one-of", "class" -> c, "model" -> model)
      ...
      m.empty match {
        case true => json merge(JObject(("empty" -> JBool(true))))
        case _ => json
      }
    }
  }
  ...
  private def d(tyep: String, props: (String, Any)*)(implicit format: Formats) = {
    ...
    Extraction.decompose(Map(elems: _*))
  }
}
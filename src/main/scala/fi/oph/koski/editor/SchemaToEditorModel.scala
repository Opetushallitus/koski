...
class SchemaToEditorModel(context: ValidationAndResolvingContext, mainSchema: ClassSchema)(implicit val user: KoskiUser) extends LocalizedHtml {
  def buildModel(schema: ClassSchema, value: AnyRef): EditorModel = {
    Context().ObjectModelBuilder(schema, true).buildObjectModel(value)
  }
  ...
  private case class Context(editable: Boolean = true, root: Boolean = true, var prototypesRequested: Set[SchemaWithClassName] = Set.empty, prototypesBeingCreated: Set[SchemaWithClassName] = Set.empty) {
    private def buildModel(obj: Any, schema: Schema): EditorModel = (obj, schema) match {
      case (o: AnyRef, t:SchemaWithClassName) => getModelBuilder(t).buildObjectModel(o)
    ...
    def getModelBuilder(t: SchemaWithClassName): ModelBuilder = t match {
      case t: ClassSchema =>
      ...
      case t: ClassRefSchema => getModelBuilder(mainSchema.getSchema(t.fullClassName).get)
    ...
    case class AnyOfModelBuilder(t: AnyOfSchema) extends ModelBuilder {
      def buildObjectModel(obj: AnyRef) = obj match {
        case None => OneOfModel(t.simpleName, None, t.alternatives.flatMap(getPrototypePlaceholder(_)))
        case x: AnyRef => OneOfModel(t.simpleName, Some(buildModel(x, findOneOfSchema(t, x))), t.alternatives.flatMap(getPrototypePlaceholder(_)))
      }
      def prototypeKey = t.simpleName
    }

    case class EnumModelBuilder[A](alternativesPath: String, toEnumValue: A => EnumValue) extends ModelBuilder {
      /*
    ...
    case class ObjectModelBuilder(schema: ClassSchema, includeData: Boolean = false) extends ModelBuilder {
          ...
          val includedPrototypes: Map[String, EditorModel] = if (root) {
            ...
            do {
              ...
              requestsFromPreviousRound.foreach { schema =>
                val helperContext = Context.this.copy(root = false, prototypesBeingCreated = Set(schema) )
                ...
                prototypesCreated += (getModelBuilder(schema).prototypeKey -> model)
              }
    ...
    private def getPrototypePlaceholder(schema: Schema): Option[EditorModel] = if (editable) {
      schema match {
        case s: SchemaWithClassName =>
          ...
          Some(getModelBuilder(s).buildPrototypePlaceholder)

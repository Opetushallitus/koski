...
Some(sql"""
    select
      ...
      count(distinct case when tuen_paatoksen_jakso then r_opiskeluoikeus.opiskeluoikeus_oid end) as tuenPäätöksenJakso,
      count(distinct case when varhennetun_oppivelvollisuuden_jakso then r_opiskeluoikeus.opiskeluoikeus_oid end) as varhennetunOppivelvollisuudenJakso,
...
"""))
  ...
  def columnSettings(t: LocalizationReader): Seq[(String, Column)] = Seq(
    ...
    "varhennetunOppivelvollisuudenJakso" -> Column(t.get("raportti-excel-kolumni-varhennetunOppivelvollisuudenJakso"), comment = Some(t.get("raportti-excel-kolumni-varhennetunOppivelvollisuudenJakso-comment"))),
...
case class EsiopetuksenOppijamäärätRaporttiRow(
  ...
  varhennetunOppivelvollisuudenJakso: Int,

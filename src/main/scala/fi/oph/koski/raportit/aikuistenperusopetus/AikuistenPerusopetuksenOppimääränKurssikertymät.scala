...
case class AikuistenPerusopetuksenOppimääränKurssikertymät(db: DB) extends QueryMethods {
    ...
    sql"""
      ...
      from (
        select
          ...
          count(distinct (case when tunnustettu_rahoituksen_piirissa and suorituksen_tyyppi = 'aikuistenperusopetuksenkurssi' then r_osasuoritus.osasuoritus_id end)) päättövaiheen_tunnistettuja_suorituksia_rahoituksen_piirissä,
  ...
  val columnSettings: Seq[(String, Column)] = Seq(
    ...
    "eriVuonnaKorotetutSuoritukset" -> Column("Suoritetut tai rahoituksen piirissä oleviksi merkityt tunnustetut kurssit - korotettu eri vuonna")
  )
}

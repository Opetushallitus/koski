...
case class PerusopetuksenLisäopetusOppijamäärätRaportti(db: DB, organisaatioService: OrganisaatioService) extends KoskiDatabaseMethods {
  ...
  val columnSettings: Seq[(String, Column)] = Seq(
    ...
    "oppilaita" -> Column("Perusopetuksen lisäopetuksen oppilaiden määrä", comment = Some("\"Läsnä\"-tilaiset perusopetuksen lisäopetuksen opiskeluoikeudet raportin tulostusparametreissa määriteltynä päivänä.")),
    ...
    "erityiselläTuella" -> Column("Perusopetusoppilaat, joilla erityinen tuki", comment = Some("Perusopetuksen lisäopetuksen oppilaat, joille löytyy opiskeluoikeuden lisätiedoista raportin tulostusparametreissa määritellylle päivälle osuva erityisen tuen jakso.")),

...
case class VstRaportitRepository(raportointiDatabase: RaportointiDatabase) extends QueryMethods with RaportointikantaTableQueries {
  ...
  private def opiskeluoikeusAikajaksotPaatasonSuorituksetResult(
    ...
    runDbSync(
      opiskeluoikeusAikajaksotPaatasonSuorituksetQuery(oppilaitosOids.toSeq, suorituksenTyyppi, alku, loppu).as[OpiskeluoikeusPtsAikajaksot],
      timeout = defaultTimeout,
    )
  ...
  private def opiskeluoikeusAikajaksotPaatasonSuorituksetQuery(
    ...
    sql"""
      ...
      WHERE
        ...
        oo.koulutusmuoto = 'vapaansivistystyonkoulutus' AND

...
trait KoskiSpecificApiServlet extends ApiServlet with KoskiSpecificBaseServlet {
  ...
  def getSessionFromRow(row: SuoritusjakoRow, config: com.typesafe.config.Config): Either[HttpStatus, KoskiSpecificSession] = {
    val erillistiedotJakolinkkeihinAlkaen = LocalDate.parse(config.getString("erillistiedotJakolinkkeihinAlkaen")).minusDays(1)
    if (row.aikaleima.toLocalDateTime.toLocalDate.isAfter(erillistiedotJakolinkkeihinAlkaen)) {
      Right(KoskiSpecificSession.suoritusjakoKatsominenErillistiedoillaUser(request))
    } else {

...
@Title("Lukio/DIA/IB/International/ESH opiskelijamäärät")
@Description("Palauttaa lukio/DIA/IB/International School/ESH opiskelijamäärät-raportin.")
case class MassaluovutusQueryLukioDiaIbInternationalESHOpiskelijamaarat(
  @EnumValues(Set("lukioDiaIbInternationalESHOpiskelijamaarat"))
  `type`: String = "lukioDiaIbInternationalESHOpiskelijamaarat",
  ...
  @Description("Päivämäärä, jolta opiskelijamäärät lasketaan.")
  paiva: LocalDate,
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.lukioDiaIbInternationalESHOpiskelijaMaaratRaportti(request, localizationReader), format, localizationReader)
  ...
  override def queryAllowed(application: KoskiApplication)(implicit user: Session): Boolean = withKoskiSpecificSession { u =>
    (u.hasGlobalReadAccess || organisaatioOid.exists(oid => u.hasRaporttiReadAccess(oid))) &&
      u.allowedOpiskeluoikeudetJaPäätasonSuoritukset.intersects(OoPtsMask(OpiskeluoikeudenTyyppi.lukiokoulutus.koodiarvo))
  }

  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryLukioDiaIbInternationalESHOpiskelijamaarat] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("lukiodiaibinternationaleshopiskelijamaarat"),
...
object QueryLukioDiaIbInternationalESHOpiskelijamaaratDocumentation {
  def xlsxExample: MassaluovutusQueryLukioDiaIbInternationalESHOpiskelijamaarat = MassaluovutusQueryLukioDiaIbInternationalESHOpiskelijamaarat(

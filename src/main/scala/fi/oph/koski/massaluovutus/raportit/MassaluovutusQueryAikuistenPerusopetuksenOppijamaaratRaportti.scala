...
case class MassaluovutusQueryAikuistenPerusopetuksenOppijamaaratRaportti(
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.aikuistenperusopetuksenOppijamäärät(request, localizationReader)(s), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryAikuistenPerusopetuksenOppijamaaratRaportti] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("aikuistenperusopetuksenoppijamaaratraportti"),

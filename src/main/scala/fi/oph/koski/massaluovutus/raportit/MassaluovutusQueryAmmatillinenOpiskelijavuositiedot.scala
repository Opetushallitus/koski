...
@Title("Ammatillinen opiskelijavuositiedot")
@Description("Palauttaa ammatillisen koulutuksen opiskelijavuositiedot-raportin.")
...
case class MassaluovutusQueryAmmatillinenOpiskelijavuositiedot(
  @EnumValues(Set("ammatillinenOpiskelijavuositiedot"))
  `type`: String = "ammatillinenOpiskelijavuositiedot",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.opiskelijaVuositiedot(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryAmmatillinenOpiskelijavuositiedot] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("ammatillinenopiskelijavuositiedot"),
...
object QueryAmmatillinenOpiskelijavuositiedotDocumentation {
  def xlsxExample: MassaluovutusQueryAmmatillinenOpiskelijavuositiedot = MassaluovutusQueryAmmatillinenOpiskelijavuositiedot(

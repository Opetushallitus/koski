...
@Title("TUVA perusopetuksen oppijamäärät")
@Description("Palauttaa TUVA-koulutuksen perusopetuksen oppijamääräraportin.")
case class MassaluovutusQueryTuvaPerusopetuksenOppijamaaratRaportti(
  @EnumValues(Set("tuvaPerusopetuksenOppijamaaratRaportti"))
  `type`: String = "tuvaPerusopetuksenOppijamaaratRaportti",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.tuvaPerusopetuksenOppijamäärät(request, localizationReader), format, localizationReader)
  ...
  override def queryAllowed(application: KoskiApplication)(implicit user: Session): Boolean = withKoskiSpecificSession { u =>
    (u.hasGlobalReadAccess || organisaatioOid.exists(oid => u.hasRaporttiReadAccess(oid))) &&
      u.allowedOpiskeluoikeudetJaPäätasonSuoritukset.intersects(OoPtsMask(OpiskeluoikeudenTyyppi.tuva.koodiarvo))
  }

  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryTuvaPerusopetuksenOppijamaaratRaportti] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("tuvaperusopetuksenoppijamaaratraportti"),
...
object QueryTuvaPerusopetuksenOppijamaaratRaporttiDocumentation {
  def xlsxExample: MassaluovutusQueryTuvaPerusopetuksenOppijamaaratRaportti = MassaluovutusQueryTuvaPerusopetuksenOppijamaaratRaportti(

...
case class MassaluovutusQueryPerusopetuksenOppijamaaratRaportti(
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.perusopetuksenOppijamäärät(request, localizationReader)(s), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryPerusopetuksenOppijamaaratRaportti] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("perusopetuksenoppijamaaratraportti"),

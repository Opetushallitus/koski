...
@Title("Lukio 2019 opintopistekertymät")
@Description("Palauttaa lukion 2019 opintopistekertymät-raportin (LOPS 2021).")
case class MassaluovutusQueryLukio2019Opintopistekertymat(
  @EnumValues(Set("lukio2019Opintopistekertymat"))
  `type`: String = "lukio2019Opintopistekertymat",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.lukio2019KoulutuksenOpintopistekertyma(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryLukio2019Opintopistekertymat] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("lukio2019opintopistekertymat"),
...
object QueryLukio2019OpintopistekertymatDocumentation {
  def xlsxExample: MassaluovutusQueryLukio2019Opintopistekertymat = MassaluovutusQueryLukio2019Opintopistekertymat(

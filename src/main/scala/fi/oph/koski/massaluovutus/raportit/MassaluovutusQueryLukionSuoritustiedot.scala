...
@Title("Lukion suoritustietojen tarkistus")
@Description("Palauttaa lukion suoritustietojen tarkistusraportin.")
...
case class MassaluovutusQueryLukionSuoritustiedot(
  @EnumValues(Set("lukionSuoritustiedot"))
  `type`: String = "lukionSuoritustiedot",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.lukioraportti(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryLukionSuoritustiedot] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("lukionsuoritustietojentarkistus"),
...
object QueryLukionSuoritustiedotDocumentation {
  def xlsxExample: MassaluovutusQueryLukionSuoritustiedot = MassaluovutusQueryLukionSuoritustiedot(

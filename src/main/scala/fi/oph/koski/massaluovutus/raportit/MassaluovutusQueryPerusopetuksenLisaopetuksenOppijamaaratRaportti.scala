...
@Title("Perusopetuksen lisäopetuksen oppijamäärät")
@Description("Palauttaa perusopetuksen lisäopetuksen oppijamääräraportin.")
case class MassaluovutusQueryPerusopetuksenLisaopetuksenOppijamaaratRaportti(
  @EnumValues(Set("perusopetuksenLisaopetuksenOppijamaaratRaportti"))
  `type`: String = "perusopetuksenLisaopetuksenOppijamaaratRaportti",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.perusopetuksenLisäopetuksenOppijamäärät(request, localizationReader), format, localizationReader)
  ...
  override def queryAllowed(application: KoskiApplication)(implicit user: Session): Boolean = withKoskiSpecificSession { u =>
    (u.hasGlobalReadAccess || organisaatioOid.exists(oid => u.hasRaporttiReadAccess(oid))) &&
      u.allowedOpiskeluoikeudetJaPäätasonSuoritukset.intersects(OoPtsMask(OpiskeluoikeudenTyyppi.perusopetuksenlisaopetus.koodiarvo))
  }

  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryPerusopetuksenLisaopetuksenOppijamaaratRaportti] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("perusopetuksenlisaopetuksenoppijamaaratraportti"),
...
object QueryPerusopetuksenLisaopetuksenOppijamaaratRaporttiDocumentation {
  def xlsxExample: MassaluovutusQueryPerusopetuksenLisaopetuksenOppijamaaratRaportti = MassaluovutusQueryPerusopetuksenLisaopetuksenOppijamaaratRaportti(

...
@Title("Vapaan sivistystyön JOTPA")
@Description("Palauttaa vapaan sivistystyön jatkuvan oppimisen ja työllisyyden palvelukeskuksen (JOTPA) raportin.")
case class MassaluovutusQueryVSTJOTPA(
  @EnumValues(Set("vstJotpa"))
  `type`: String = "vstJotpa",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.vstJotpa(request, localizationReader), format, localizationReader)
  ...
  override def queryAllowed(application: KoskiApplication)(implicit user: Session): Boolean = withKoskiSpecificSession { u =>
    (u.hasGlobalReadAccess || organisaatioOid.exists(oid => u.hasRaporttiReadAccess(oid))) &&
      u.allowedOpiskeluoikeudetJaPäätasonSuoritukset.intersects(OoPtsMask(OpiskeluoikeudenTyyppi.vapaansivistystyonkoulutus.koodiarvo))
  }

  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryVSTJOTPA] =
    for {
...
object QueryVSTJOTPADocumentation {
  def xlsxExample: MassaluovutusQueryVSTJOTPA = MassaluovutusQueryVSTJOTPA(

...
@Title("Perusopetukseen valmistavan opetuksen suoritustietojen tarkistus")
@Description("Palauttaa perusopetukseen valmistavan opetuksen suoritustietojen tarkistusraportin.")
case class MassaluovutusQueryPerusopetukseenValmistava(
  @EnumValues(Set("perusopetukseenValmistava"))
  `type`: String = "perusopetukseenValmistava",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.perusopetukseenvalmistavanopetuksenraportti(request, localizationReader), format, localizationReader)
  ...
  override def queryAllowed(application: KoskiApplication)(implicit user: Session): Boolean = withKoskiSpecificSession { u =>
    (u.hasGlobalReadAccess || organisaatioOid.exists(oid => u.hasRaporttiReadAccess(oid))) &&
      u.allowedOpiskeluoikeudetJaPäätasonSuoritukset.intersects(OoPtsMask(OpiskeluoikeudenTyyppi.perusopetukseenvalmistavaopetus.koodiarvo))
  }

  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryPerusopetukseenValmistava] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("perusopetukseenvalmistavansuoritustietojentarkistus"),
...
object QueryPerusopetukseenValmistavaDocumentation {
  def xlsxExample: MassaluovutusQueryPerusopetukseenValmistava = MassaluovutusQueryPerusopetukseenValmistava(

...
@Title("TUVA suoritustietojen tarkistus")
@Description("Palauttaa TUVA-koulutuksen suoritustietojen tarkistusraportin.")
case class MassaluovutusQueryTuvaSuoritustiedot(
  @EnumValues(Set("tuvaSuoritustiedot"))
  `type`: String = "tuvaSuoritustiedot",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.tuvaSuoritustiedot(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryTuvaSuoritustiedot] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("tuvasuoritustietojentarkistus"),
...
object QueryTuvaSuoritustiedotDocumentation {
  def xlsxExample: MassaluovutusQueryTuvaSuoritustiedot = MassaluovutusQueryTuvaSuoritustiedot(

...
@Title("Aikuisten perusopetuksen kurssikertymät")
@Description("Palauttaa aikuisten perusopetuksen kurssikertymät-raportin.")
case class MassaluovutusQueryAikuistenPerusopetuksenKurssikertyma(
  @EnumValues(Set("aikuistenPerusopetuksenKurssikertyma"))
  `type`: String = "aikuistenPerusopetuksenKurssikertyma",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      val s = user.asInstanceOf[KoskiSpecificSession]

      val localizationReader = new LocalizationReader(application.koskiLocalizationRepository, language.get)
      writer.putReport(raportitService.aikuistenperusopetuksenKurssikertymä(request, localizationReader)(s), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryAikuistenPerusopetuksenKurssikertyma] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("aikuistenperusopetuksenkurssikertymaraportti"),
...
object QueryAikuistenPerusopetuksenKurssikertymaDocumentation {
  def xlsxExample: MassaluovutusQueryAikuistenPerusopetuksenKurssikertyma = MassaluovutusQueryAikuistenPerusopetuksenKurssikertyma(

...
case class MassaluovutusQueryAmmatillinenTutkintoSuoritustiedot(
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.ammatillinenTutkintoSuoritustietojenTarkistus(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryAmmatillinenTutkintoSuoritustiedot] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("ammatillinentutkintosuoritustietojentarkistus"),
...
object QueryAmmatillinenTutkintoSuoritustiedotDocumentation {
  def xlsxExample: MassaluovutusQueryAmmatillinenTutkintoSuoritustiedot = MassaluovutusQueryAmmatillinenTutkintoSuoritustiedot(

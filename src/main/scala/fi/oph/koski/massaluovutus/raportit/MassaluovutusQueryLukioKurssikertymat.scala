...
@Title("Lukion kurssikertymät")
@Description("Palauttaa lukion kurssikertymät-raportin.")
case class MassaluovutusQueryLukioKurssikertymat(
  @EnumValues(Set("lukioKurssikertymat"))
  `type`: String = "lukioKurssikertymat",
  ...
  override def run(application: KoskiApplication, writer: QueryResultWriter)(implicit user: Session with SensitiveDataAllowed): Either[String, Unit] =
    QueryResourceManager(logger) { mgr =>
      ...
      writer.putReport(raportitService.lukioKoulutuksenKurssikertyma(request, localizationReader), format, localizationReader)
  ...
  override def fillAndValidate(implicit user: Session): Either[HttpStatus, MassaluovutusQueryLukioKurssikertymat] =
    for {
  ...
  private def auditLog(implicit user: Session): Unit =
    AuditLog.log(KoskiAuditLogMessage(
      ...
      Map(hakuEhto -> OpiskeluoikeusQueryContext.queryForAuditLog(Map(
        "raportti" -> List("lukiokurssikertymat"),
...
object QueryLukioKurssikertymatDocumentation {
  def xlsxExample: MassaluovutusQueryLukioKurssikertymat = MassaluovutusQueryLukioKurssikertymat(

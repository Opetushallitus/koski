...
import fi.oph.koski.koskiuser.{KoskiSpecificAuthenticationSupport, LuovutuspalveluAuthenticationSupport, RequiresKansalainen}
...
class OmaDataOAuth2ResourceOwnerServlet(implicit val application: KoskiApplication)
  extends KoskiSpecificApiServlet with KoskiSpecificAuthenticationSupport with LuovutuspalveluAuthenticationSupport
    with Logging with ContentEncodingSupport with NoCache with LanguageSupport with OmaDataOAuth2ServletSupport with RequiresKansalainen {

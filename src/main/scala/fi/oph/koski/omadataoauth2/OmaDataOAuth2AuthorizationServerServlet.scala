...
class OmaDataOAuth2AuthorizationServerServlet(implicit val application: KoskiApplication)
  ...
  post("/") {
    val result: AccessTokenResponse = validate(AccessTokenRequest.formAccessTokenRequest)(
      (errors: Seq[(String, String)]) => {
        val validationError = ValidationError(ValidationErrorType.invalid_request, errors.map { case (a, b) => s"${a}: ${b}" }.mkString(";"), ReportingType.ToRedirectUri)
  ...
  protected def validateClientIdSamaKuinKäyttäjätunnus(clientId: String): Either[ValidationError, String] = {
    if (koskiSession.user.username == clientId) {
      ...
      Left(ValidationError(ValidationErrorType.invalid_client_data, s"Annettu client_id ${clientId} on eri kuin mTLS-käyttäjä ${koskiSession.user.username}", ReportingType.ToRedirectUri))
    }
  }
}

...
class OpiskeluoikeusServlet(implicit val application: KoskiApplication) extends KoskiSpecificApiServlet with RequiresVirkailijaOrPalvelukäyttäjä with Logging with NoCache with Timing {
  get("/:oid") {
    val mySession = timed("get-oo-by-oid-investigation:session") {
      session
    }
    val result: Either[HttpStatus, KoskiOpiskeluoikeusRow] =
      timed("get-oo-by-oid-investigation:application.opiskeluoikeusRepository.findByOid(getStringParam(\"oid\"))(session)") {
        application.opiskeluoikeusRepository.findByOid(getStringParam("oid"))(mySession)
      }


    timed("get-oo-by-oid-investigation:auditlog") {
    ...
    val unsafe = timed("get-oo-by-oid-investigation:result.map(_.toOpiskeluoikeusUnsafe)") {

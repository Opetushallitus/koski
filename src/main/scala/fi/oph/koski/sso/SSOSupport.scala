...
trait SSOSupport extends ScalatraBase with Logging {
  ...
  def casOppijaLogoutUrl(service: String = koskiRoot) = {
    application.config.getString("opintopolku.oppija.url") + "/cas-oppija/logout?service=" + service
  }

...
class CasService(config: Config) extends Logging {
  ...
  def validateKansalainenServiceTicket(url: String, ticket: String): KansalaisenTunnisteet = {
    ...
    // TODO: vain testaukseen Devillä/QA:lla:
    logger.info(s"validateKansalainenServiceTicket oppijaAttributes: ${oppijaAttributes.mkString(", ")}")

    val hetuAttempt = oppijaAttributes.get(ATTRIBUTE_NAME_HETU).filter(_.nonEmpty) // HETU
    val oppijaOidAttempt = oppijaAttributes.get(ATTRIBUTE_NAME_PERSON_OID).filter(_.nonEmpty) // OppijaOid

...
object OpiskeluoikeusLoader extends Logging {
  ...
  def loadOpiskeluoikeudet(opiskeluoikeusQueryRepository: OpiskeluoikeusQueryService, systemUser: KoskiSession, db: RaportointiDatabase, batchSize: Int = DefaultBatchSize): Observable[LoadResult] = {
    ...
    val result = opiskeluoikeusQueryRepository.mapKaikkiOpiskeluoikeudetSivuittain(batchSize, systemUser) { opiskeluoikeusRows =>
      if (opiskeluoikeusRows.nonEmpty) {
        ...
        val aikuistenPerusopetusOpiskeluoikeusAikajaksoRows = outputRows.flatMap(_.right.get.aikuistenPerusopetusOpiskeluoikeusAikajaksoRows)
        ...
        db.loadAikuistenPerusopetusOpiskeluoikeusAikajaksot(aikuistenPerusopetusOpiskeluoikeusAikajaksoRows)
  ...
  private def deleteEverything(raportointiDatabase: RaportointiDatabase): Unit = {
    ...
    raportointiDatabase.deleteAikuistenPerusopetusOpiskeluoikeusAikajaksot
  }
  ...
  private def buildRow(inputRow: OpiskeluoikeusRow): Either[LoadErrorResult, OutputRows] = {
    Try {
      ...
      OutputRows(
        ...
        aikuistenPerusopetusOpiskeluoikeusAikajaksoRows = aikajaksoRows._3,
      )
  ...
  private[raportointikanta] def buildSuoritusRows(opiskeluoikeusOid: String, oppilaitos: OrganisaatioWithOid, ps: PäätasonSuoritus, data: JValue, idGenerator: => Long) = {
    ...
    val päätaso = buildRPäätasonSuoritusRow(opiskeluoikeusOid, oppilaitos, ps, data, päätasonSuoritusId)
  ...
  private def buildRPäätasonSuoritusRow(opiskeluoikeusOid: String, oppilaitos: OrganisaatioWithOid, ps: PäätasonSuoritus, data: JValue, päätasonSuoritusId: Long) = {
...
case class OutputRows(
  ...
  aikuistenPerusopetusOpiskeluoikeusAikajaksoRows: Seq[AikuistenPerusopetuksenOpiskeluoikeusAikajaksoRow],
)

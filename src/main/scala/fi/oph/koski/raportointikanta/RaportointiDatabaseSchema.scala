...
object RaportointiDatabaseSchema {
  ...
  class RKotikuntahistoriaTable(tag: Tag, schema: Schema = Public) extends Table[RKotikuntahistoriaRow](tag, schema.nameOpt, "r_kotikuntahistoria") {
    ...
    val kotikunnanNimiFi = column[String]("kotikunta_nimi_fi")
    val kotikunnanNimiSv = column[String]("kotikunta_nimi_sv")
...
class RGeneerinenAikajaksoTable(tag: Tag, schema: Schema = Public) extends Table[RGeneerinenAikajaksoRow](tag, schema.nameOpt, "r_geneerinen_aikajakso") {
  ...
  def * = (
    ...
    alku,
    loppu
  ) <> (RGeneerinenAikajaksoRow.tupled, RGeneerinenAikajaksoRow.unapply)
}

class RGeneerinenAikajaksoTableTemp(tag: Tag) extends RGeneerinenAikajaksoTable(tag, Temp)

class RGeneerinenAikajaksoConfidentialTable(tag: Tag) extends RGeneerinenAikajaksoTable(tag, Confidential)

class RGeneerinenAikajaksoConfidentialTableTemp(tag: Tag) extends RGeneerinenAikajaksoTable(tag, TempConfidential)

class RAmmatillisenKoulutuksenJarjestamismuotoAikajaksoTable(tag: Tag, schema: Schema = Public) extends Table[RAmmatillisenKoulutuksenJarjestamismuotoAikajaksoRow](tag, schema.nameOpt, "r_ammatillisen_koulutuksen_jarjestamismuoto") {
  val opiskeluoikeusOid = column[String]("opiskeluoikeus_oid")
  val aikajaksoTyyppi = column[String]("aikajakso_tyyppi")
  val alku = column[Date]("alku")
  val loppu = column[Option[Date]]("loppu")
...
case class RGeneerinenAikajaksoRow(
                                   opiskeluoikeusOid: String,
...
sealed trait Schema {
  ...
  def createOpiskeluoikeusIndexes() = DBIO.seq(
    ...
    sqlu"CREATE INDEX ON #${name}.r_geneerinen_aikajakso(opiskeluoikeus_oid, aikajakso_tyyppi, alku, loppu)",
    sqlu"CREATE INDEX ON #${name}.r_geneerinen_aikajakso(aikajakso_tyyppi)",

    sqlu"CREATE INDEX ON #${name}.r_ammatillisen_koulutuksen_jarjestamismuoto(opiskeluoikeus_oid, aikajakso_tyyppi, alku, loppu)",
    sqlu"CREATE INDEX ON #${name}.r_ammatillisen_koulutuksen_jarjestamismuoto(aikajakso_tyyppi)",

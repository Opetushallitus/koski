...
object RaportointiDatabaseSchema {
  ...
  class ROpiskeluoikeusAikajaksoTable(tag: Tag, schema: Schema = Public) extends Table[ROpiskeluoikeusAikajaksoRow](tag, schema.nameOpt, "r_opiskeluoikeus_aikajakso") {
    ...
    val maksullinen = column[Option[Boolean]]("maksullinen")
    ...
    val valmistavanLisäopetus = column[Boolean]("valmistavan_lisaopetus")
...
case class ROpiskeluoikeusAikajaksoRow(
  ...
  valmistavanLisäopetus: Boolean = false,
  id: Long = 0
...
case class RPäätasonSuoritusRow(
  ...
  def omanÄidinkielenOpinnotKieliDatasta: Option[LocalizedString] =
    JsonSerializer.extract[Option[LocalizedString]](data \ "omanÄidinkielenOpinnot" \ "kieli" \ "nimi")

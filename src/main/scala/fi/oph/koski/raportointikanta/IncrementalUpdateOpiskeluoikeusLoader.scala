...
class IncrementalUpdateOpiskeluoikeusLoader(
  ...
  private def updateBatchOlemassaolevatOpiskeluoikeudet(
    ...
    val opiskelioikeusAikajaksoRows = outputRows.flatMap(_.right.get.rOpiskeluoikeusAikajaksoRows)
    db.updateOpiskeluoikeusAikajaksot(opiskelioikeusAikajaksoRows)

...
object MockUsers {
  ...
  val users = List(kalle, omniaPalvelukäyttäjä, omniaKatselija, omniaTallentaja, localkoski, paakayttaja, viranomainen, stadinAmmattiopistoPalvelukäyttäjä, stadinAmmattiopistoTallentaja, helsinkiPalvelukäyttäjä, kahdenOrganisaatioPalvelukäyttäjä)
}

...
object Käyttöoikeus {
  def parseAllowedOpiskeluoikeudenTyypit(roolit: List[Palvelurooli], accessTypes: List[AccessType.Value]): Set[String] = {
    ...
    val kayttajanRoolit = supportOldWayOfDefiningTyyppikohtainenOikeus(roolit).filter(_.palveluName == "KOSKI").map(_.rooli.toLowerCase).toSet
  ...
  private def supportOldWayOfDefiningTyyppikohtainenOikeus(roolit: List[Palvelurooli]) = roolit flatMap {
    ...
    case Palvelurooli("KOSKI", READ_UPDATE_ESIOPETUS) => List(Palvelurooli(READ_UPDATE), Palvelurooli(ESIOPETUS))
    case Palvelurooli("KOSKI", LUKU_ESIOPETUS) => List(Palvelurooli(READ), Palvelurooli(ESIOPETUS))
    case Palvelurooli("KOSKI", GLOBAALI_LUKU_KORKEAKOULU) => List(Palvelurooli(READ), Palvelurooli(KORKEAKOULUTUS))

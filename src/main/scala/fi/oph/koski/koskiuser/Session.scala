...
class KoskiSpecificSession(
  ...
  lazy val varhaiskasvatusKoulutustoimijat: Set[Oid] = varhaiskasvatuksenOstopalvelukäyttöoikeudet.map(_.koulutustoimija.oid)
  ...
  def hasRaporttiReadAccess(organisaatio: Organisaatio.Oid): Boolean = {
    hasReadAccess(organisaatio, None) ||
      varhaiskasvatuksenOstopalvelukäyttöoikeudet
        .exists(oikeus =>
          oikeus.varhaiskasvatusToimipisteet.exists(oid => oid == organisaatio && hasReadAccess(organisaatio, Some(oikeus.koulutustoimija.oid)))
        )
  }
  ...
  def hasVarhaiskasvatusAccess(koulutustoimijaOid: Organisaatio.Oid, organisaatioOid: Organisaatio.Oid, accessType: AccessType.Value): Boolean = {
    val onOikeusKoulutustoimijanKautta = varhaiskasvatuksenOstopalvelukäyttöoikeudet.exists(
      k => k.koulutustoimija.oid == koulutustoimijaOid &&
        k.organisaatioAccessType.contains(accessType) && k.kaikkiToimipaikat.contains(organisaatioOid)
    )

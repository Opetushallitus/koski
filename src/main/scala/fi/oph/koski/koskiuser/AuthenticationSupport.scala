...
trait AuthenticationSupport extends BaseServlet with SSOSupport {
  ...
  def localLogin(user: AuthenticationUser, lang: Option[String] = None): AuthenticationUser = {
    ...
    // Kansalaisen kielivalinta talletetaan evästeeseen; virkailijan kieli ratkaistaan pyyntökohtaisesti.
    lang.foreach(UserLanguage.setLanguageCookie(_, response))

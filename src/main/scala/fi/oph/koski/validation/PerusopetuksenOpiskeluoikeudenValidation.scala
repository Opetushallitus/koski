  ...
  def fillPerusopetuksenVuosiluokkiinSitoutumatonOpetus(config: Config)(oo: KoskeenTallennettavaOpiskeluoikeus): KoskeenTallennettavaOpiskeluoikeus = {
    ...
    oo match {
      case p: PerusopetuksenOpiskeluoikeus =>
        val normalized: Option[Boolean] =
          (p.alkamispäivä, p.lisätiedot.flatMap(_.vuosiluokkiinSitoutumatonOpetus)) match {
            case (Some(d), None) if !d.isAfter(lastAllowed) => Some(false) // pre-cutoff, lisätiedot exists but field empty
            case (_, v)                                     => v            // leave as-is (including None)
          }
        p.copy(lisätiedot = p.lisätiedot.map(_.copy(vuosiluokkiinSitoutumatonOpetus = normalized)))
      case other => other
    }
  }
  ...
  def validateVuosiluokkiinSitoutumatonOpetusEiSallittu(config: Config, oo: KoskeenTallennettavaOpiskeluoikeus): HttpStatus = {
    ...
    oo match {
      case p: PerusopetuksenOpiskeluoikeus =>
        ...
        } else if (p.lisätiedot.nonEmpty && (p.alkamispäivä.exists(_.isBefore(lastAllowed)) || p.alkamispäivä.exists(_.isEqual(lastAllowed))) && p.lisätiedot.flatMap(_.vuosiluokkiinSitoutumatonOpetus).isEmpty) {
          KoskiErrorCategory.badRequest.validation.rakenne.vsopEnnenPäivämäärääPuuttuu()
        }else{
          HttpStatus.ok
        }
      case _ => HttpStatus.ok

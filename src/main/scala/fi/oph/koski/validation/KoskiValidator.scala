...
class KoskiValidator(tutkintoRepository: TutkintoRepository, val koodistoPalvelu: KoodistoViitePalvelu, val organisaatioRepository: OrganisaatioRepository, koskiOpiskeluoikeudet: KoskiOpiskeluoikeusRepository, henkilöRepository: HenkilöRepository, ePerusteet: EPerusteetRepository, config: Config) extends Timing {
  ...
  private def validateSuoritus(suoritus: Suoritus, opiskeluoikeus: KoskeenTallennettavaOpiskeluoikeus, parent: List[Suoritus])(implicit user: KoskiSession, accessType: AccessType.Value): HttpStatus = {
    ...
    HttpStatus.fold(
      ...
      validateDateOrder(alkamispäivä, ("suoritus.arviointi.päivä", arviointipäivät), KoskiErrorCategory.badRequest.validation.date.arviointiEnnenAlkamispäivää)
        ...
        :: validateLukio2019Osasuoritukset(suoritus, parent)
  ...
  private def validateLukio2019Osasuoritukset(suoritus: Suoritus, parent: List[Suoritus]): HttpStatus = {
    ...
    def validateErityinenTutkinto(suoritus: Suoritus, parent: List[Suoritus]): HttpStatus = (suoritus, parent) match {
    ...
    def validateTemaattisissaVainPaikallisiaOpintojaksoja(suoritus: Suoritus, parent: List[Suoritus]): HttpStatus = (suoritus, parent) match {
    ...
    def validateLukiodiplomiRakenne(suoritus: Suoritus, parent: List[Suoritus]): HttpStatus = (suoritus, parent) match {
    ...
    def validateLukiodiplomiLaajuus(suoritus: Suoritus): HttpStatus = suoritus match {
    ...
    HttpStatus.fold(List(
      validateErityinenTutkinto(suoritus, parent),
      validateTemaattisissaVainPaikallisiaOpintojaksoja(suoritus, parent),
      validateLukiodiplomiRakenne(suoritus, parent),

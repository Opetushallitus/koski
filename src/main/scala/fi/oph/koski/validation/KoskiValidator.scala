    ...
    validateAccess(opiskeluoikeus)
      ...
      .onSuccess {
        HttpStatus.fold(
          ...
          AmmatillinenValidation.validateAmmatillinenOpiskeluoikeus(config)(opiskeluoikeus, henkilö, koskiOpiskeluoikeudet),
 ...
 private def validateNuortenPerusopetuksenPakollistenOppiaineidenLaajuus(suoritus: Suoritus, opiskeluoikeus: KoskeenTallennettavaOpiskeluoikeus)  = {
  ...
  private def validateSuorituskieli(suoritus: Suoritus) = {
    ...
    suoritus match {
      ...
      case k: IBTutkinnonSuoritus => HttpStatus.validate(k.suorituskieli.koodiarvo == "EN") {
        KoskiErrorCategory.badRequest.validation.rakenne.virheellinenSuorituskieli("IB-tutkinnon suorituksen suorituskielen tulee olla englanti")
      }
      case _ => HttpStatus.ok
    }
  }

...
import fi.oph.koski.koskiuser.KoskiSpecificSession.untrustedUser
...
object SchemaToJsonHtml {
  ...
  private def buildHtml(property: Property, obj: Any, schema: ClassSchema, context: NodeContext): List[Elem] = (obj, property.schema) match {
    ...
    case (x: AnyRef, t:ClassRefSchema) => buildHtml(property.copy(schema = t.resolve(KoskiSchema.schemaFactory, schema)), x , schema, context)
    case _ =>

...
object ExamplesYlioppilastutkinto {
  ...
  val opiskeluoikeus = MockYrtClient.oppijaByHetu(MockOppijat.ylioppilas.hetu.get).flatMap(YtrOppijaConverter(oppilaitokset, koodistoViitePalvelu, MockOrganisaatioRepository, MockLocalizationRepository(LocalizationConfig("koski"))).convert(_)).get

...
object ComputedPropertyContext {
  ...
  /**
    * Laskettavat kentät voivat käyttää tätä kontekstia vain synkronisessa työssä,
    * joka alkaa sen jälkeen kun withOptionalContext on asettanut currentContextin.
    *
    * currentContext on DynamicVariable: arvo näkyy samassa säikeessä ja niissä
    * lapsisäikeissä, jotka käynnistetään kyseisestä säikeestä asettamisen jälkeen,
    * mutta ei muussa asynkronisessa työssä.
    */
  ...
  // Asettaa laskettavien kenttien dynaamisen kontekstin ohjelmalohkon ajaksi

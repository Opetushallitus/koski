...
import fi.oph.koski.schema.AnyOfDeserialization.{DiscriminatorCriterion, extractAnyOf}
import fi.oph.scalaschema._
import fi.oph.scalaschema.annotation._
import org.json4s._

case class DeserializationContext(rootSchema: SchemaWithClassName, path: String = "", customDeserializers: List[CustomDeserializer] = Nil) {
...
case class TooManyMatchingCasesException(path: String, cases: List[(SchemaWithClassName, List[DiscriminatorCriterion])]) extends RuntimeException(s"Deserialization error at ${path}: more than 1 matching case: \n${cases.map {
...
object SchemaBasedJsonDeserializer {
  ...
  def extractList(json: JValue, as: ListSchema, metadata: List[Metadata])(implicit context: DeserializationContext): Either[List[ValidationError], Any] = json match {
    case JArray(values) =>
      ...
      val metadataValidationErrors: List[ValidationError] = (as.metadata ++ metadata).collect {
  ...
  private def extractNumber(json: JValue, schema: NumberSchema, metadata: List[Metadata])(implicit context: DeserializationContext): Either[List[ValidationError], Number] = {
    ...
    extractionResult.right.flatMap { number: Number =>
      ((metadata ++ schema.metadata).collect {
  ...
  private def extractString(json: JValue, schema: StringSchema, metadata: List[Metadata])(implicit context: DeserializationContext): Either[List[ValidationError], String] = json match {
    case JString(s) =>
      ...
      stringValue match {
        ...
        case _ =>
          val errors = (schema.metadata ++ metadata).collect {
    ...
    matchingSchemas match {
      ...
      case _ =>
        ...
        schemasWithMaximumNumberOfMatchingCriteria match {
          case List((schema, criteria)) => SchemaBasedJsonDeserializer.extract(json, schema, metadata)

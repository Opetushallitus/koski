...
case class PerusopetuksenOpiskeluoikeudenLisätiedot(
  ...
  päätösToimintaAlueittainOpiskelusta: Option[Aikajakso] = None,
  ...
  with VammaSairausTaiRajoite
{
  ...
  def kaikkiTukeaKoskevienPäätöstenAikajaksot: List[MahdollisestiAlkupäivällinenJakso] = {
    tukeaKoskevatPäätökset.getOrElse(List.empty)
  }
}
...
case class NuortenPerusopetuksenOppiaineenSuoritus(
  ...
  suoritustapa: Option[Koodistokoodiviite] = None
) extends PerusopetuksenOppiaineenSuoritus with OppiaineenTaiToiminta_AlueenSuoritus with Vahvistukseton with Yksilöllistettävä with RajattavaOppimäärä with MahdollisestiSuorituskielellinen with SuoritustapanaMahdollisestiErityinenTutkinto

...
object PerusopetuksenOpiskeluoikeus {
  ...
  val oppimääränArvioinnitTraversal: Traversal[Suoritus, Option[List[Arviointi]]] = traversal[Suoritus]
    .ifInstanceOf[NuortenPerusopetuksenOppimääränSuoritus]
  ...
  val pakollistenOppiaineidenLaajuudetTraversal: Traversal[Suoritus, Option[LaajuusVuosiviikkotunneissa]] = traversal[Suoritus]
    .field[Option[List[Suoritus]]]("osasuoritukset").items.items
...
case class PerusopetuksenOpiskeluoikeudenLisätiedot(
  ...
  @DefaultValue(None)
  @Deprecated("perusopetuksen aloittamista lykatty ei kaytossa")
  ...
  @Description("opetuksen järjestäminen käyttöönotto")
  ...
  @Description("erityisen tuen päätökset poistuu")
  ...
  @Deprecated("kayta korvaavaa kotiopetusjaksot")
...
) extends PerusopetuksenPäätasonSuoritus with Luokallinen with Todistus with Arvioinniton with ErityisopetuksellinenPäätasonSuoritus with LuokalleJääntiTiedonSisältäväSuoritus {
...
trait Uskonto {
  ...
  @Deprecated("uskonnon oppimaaraa ei tallenneta")

...
case class InternationalSchoolOpiskeluoikeus(
  ...
  def withLisääPuuttuvaMaksuttomuustieto = {
    lazy val uudellaMaksuttomuustiedolla = this.copy(
      lisätiedot = Some(
        this.lisätiedot.getOrElse(InternationalSchoolOpiskeluoikeudenLisätiedot())
          .copy(
...
trait InternationalSchoolVuosiluokanSuoritus extends KoskeenTallennettavaPäätasonSuoritus with Toimipisteellinen with Arvioinniton with Suorituskielellinen with MahdollisestiLuokallinen {
  @Title("Koulutus")

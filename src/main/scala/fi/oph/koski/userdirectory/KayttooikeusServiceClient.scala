...
case class KäyttöoikeusServiceClient(config: Config) {
  ...
  def getKäyttäjätiedot(oid: String): IO[Option[Käyttäjätiedot]] = http.get(uri"/kayttooikeus-service/henkilo/$oid/kayttajatiedot")(Http.parseJsonOptional[Käyttäjätiedot])
    ...
    http.get(uri"/kayttooikeus-service/kayttooikeus/KOSKI")(parseJson[List[KäyttöoikeusRooli]])
      .flatMap { roolit =>
        roolit.parTraverse { r =>
          http.post(uri"/kayttooikeus-service/kayttooikeusryhma/ryhmasByKayttooikeus", Map("KOSKI" -> r.rooli))(Json4sHttp4s.json4sEncoderOf[Map[String, String]])(Http.parseJson[List[KäyttöoikeusRyhmä]])
        }
      }.map(_.flatten)
  }

  def findKäyttöoikeusRyhmänHenkilöt(ryhmäId: Int): IO[List[String]] = http.get(uri"/kayttooikeus-service/kayttooikeusryhma/$ryhmäId/henkilot")(parseJson[KäyttöoikeusRyhmäHenkilöt]).map(_.personOids.getOrElse(List.empty))

  def findKäyttöoikeudetByUsername(username: String): IO[List[HenkilönKäyttöoikeudet]] = http.get(uri"/kayttooikeus-service/kayttooikeus/kayttaja?username=$username")(parseJson[List[HenkilönKäyttöoikeudet]])
}

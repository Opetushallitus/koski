...
function create_version() {
  ...
  export KOSKI_VERSION="$version"
  if [ "$version" == "local" ]; then
    (cd $BASE_DIR/target/dist && KOSKI_VERSION="$version" mvn install -DskipTests=true)
  else
    (cd $BASE_DIR/target/dist && mvn versions:set -DnewVersion=$version)
    (cd $BASE_DIR/target/dist && make clean && KOSKI_VERSION="$version" mvn deploy -DskipTests=true)
    git tag $version
